...
if BUILT.is_file():
    ...
    # 2026-09-03、配点スライダー導入で「出席なし」「小テストなし」の定義が
    # 変わった（scoring.passes_caps の 0% と同義にした）。数字はその後の実測。
    #   出席なし     1536 → 2170  「毎回小テストが無いこと」の要求を外したため
    #   小テストなし 6453 → 5939  weekly_quiz（本文の正規表現）ではなく配点で
    #                             判定するようにしたため
    #   レポートのみ  482 →  482  集合ごと変わっていない
    # 前2つは、内訳の合計が EVAL_TOTAL_MIN に届かない科目を落としたぶんも含む。
    # 持ち込み可 18 は main 側の変更（科目コードの先頭の0を復元した #105）。
    #
    # 2026-09-07、試験のルールから裸の「期末」「中間」を外した（scrape/parse.py）。
    # 3つとも**増えただけで、抜けた科目は1件も無い**（実測 -0）。判定が緩んだのではなく、
    # 内訳が最後まで読めるようになった科目が入ってきた。
    #   レポートのみ  482 →  508  「期末レポート」「期末課題」が試験から外れた。
    #                             例：アメリカ文学Ⅱ講義は3項目とも中身がレポート
    #   出席なし     2170 → 2176  フランス語13(A)ほか。`レベルチェック（春夏学期）`が
    #                             未分類で、内訳が最後まで読めず**チップから外れていた**
    #                             （判定④）。試験へ入って合計100%に届いた
