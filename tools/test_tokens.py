...
CONTRAST = [
    ...
    # 押せる授業内容タグの枠（button.subjTag、2026-09-21）。枠線は図形なので 3.0 でよい。
    ("押せるタグの枠",              "--muted",            "--dim",              3.0),
    # 集計の数字。--dim の上に --ink（2026-09-06 のモーダル化で足した組み合わせ）。
    ("集計の数字",                  "--ink",              "--dim",              4.5),
    # 口コミの一言に添える引用符（.dQuote::before、2026-09-10）。30px の飾りなので
    # 図形として 3.0 でよい。本文（.dQuote）自体は上の「集計の数字」と同じ
    # --ink on --dim なので、ここには別途登録していない。
    ("口コミの一言の引用符",        "--muted",            "--dim",              3.0),
    ...
    # この2行はもともと「口コミの吹き出し」（2026-08-24、--brand-soft の地に
    # 引用符と尻尾）用だったが、その吹き出しごと 2026-09-07 に detailHtml から
    # 外れ、以後は使われていない。「口コミの吹き出しの文字」の --ink on
    # --brand-soft 自体は今も .picksH（推薦枠の見出し）が使っているので改名して
    # 残し、対になっていた「吹き出しの引用符」（--brand on --brand-soft）は
    # 他に使う場所が無いので削除した。2026-09-10、口コミの一言は --dim の地に
    # 引用符を添える「そっと囲む」案で復活（下の「口コミの一言の引用符」）。
    ("推薦枠の見出し文字",          "--ink",              "--brand-soft",       4.5),
    ...
    ("内訳バー 1本目",              "--comp-1",           "--dim",              3.0),
    ("内訳バー 2本目",              "--comp-2",           "--dim",              3.0),
    ("内訳バー 3本目",              "--comp-3",           "--dim",              3.0),
    ("内訳バー 4本目",              "--comp-4",           "--dim",              3.0),
    ("内訳バー 5本目",              "--comp-5",           "--dim",              3.0),
    ("内訳バー 記載なし",           "--comp-gap",         "--dim",              3.0),
