...
if BUILT.is_file():
    ...
    # しきい値のほうを分布に合わせ直す。
    #
    # 2026-09-11: 参照値を入れ替えた。**旧参照値（5軸化直前・判定できた
    # 3,605件に対する 0.735/0.124/0.107/0.034）は退役。**
    #
    # 上の但し書き「新しい根拠は何も無いのに軽い判定が増える」は、軸を足した
    # ときの話であって、今回は当てはまらない。レポート軸が全7,906件の98%で
    # None を返して死んでいたのを生き返らせたので、**4,121件は本当に新しく
    # 判定できるようになった**（判定できた件数 3,605 → 7,474件）。
    # 母体そのものが変わった以上、旧参照値とは比較できない。
    #
    ...
    # 「古い点数 × 新しいしきい値」という意味のない比較になる（2026-09-11）。
    judged = [s for s in (scoring.score(c)["overall"] for c in courses)
              if s is not None]
