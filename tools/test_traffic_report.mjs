...
/* 1本しか来なかった日でも「誰の・どの部屋の」は出す ―― 個人配布とオプチャは
   そこが知りたい欄。配布が1本だけの X には付けない（「X 4（ポスト 4）」は無駄）。 */
check(/個人配布\D*1（笠井 1）/.test(rep), `個人配布の内訳（誰の分か）が出ていない:\n${rep}`);
check(/オプチャ\D*2（oc1 2）/.test(rep), "オプチャの内訳（どの部屋か）が出ていない");
check(!/X 4（/.test(rep), "配布が1本だけの X に無駄な内訳が付いている");
