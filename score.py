...
# ═══════════════════════════════════════════════════════════════
# 学生の言葉での絞り込み条件（サイトの「条件」チップ）
#
# 2026-09-23: server.py から**ここへ移した**。LINE bot の選択肢をサイトの
# 条件に揃える（きむら依頼）にあたり、build.py も同じ判定を使う必要が出た。
# server.py は build.py を import しているので、build.py から server.py を
# 読むと循環する。判定の土台（passes_caps）があるこのファイルが正本。
#   ・server.py          … CONDITIONS をそのまま使う
#   ・build.py           … cond_top（LINE が読む上位100件）を焼くのに使う
#   ・web/assets/app.js  … 同じ判定を JS で持つ（画面側。片方だけ直さないこと）
# ═══════════════════════════════════════════════════════════════
...
# 判定を2つ持つと必ず食い違うので、実装は passes_caps の1つだけにする。
#
#   出席なし     … 出席・平常点 0%
#   小テストなし … 小テスト 0%
...
def _chip(caps: dict):
    full = {k: NO_CAP for k in CAP_AXES} | caps
    return lambda c: passes_caps(c, full)
...
# LINE bot が選択肢に出す条件。「口コミあり」を外しているのは、サイトでも
# この7つが「条件」の枠で、口コミは別の枠に置かれているため（画面と同じ並び）。
LINE_CONDITIONS = [c for c in CONDITIONS if c != "口コミあり"]
