...
- 条件名（`出席なし` / `レポートのみ` / `持ち込み可` / `1限以外` / `集中講義` /
  `小テストなし` / `発表なし`）を含む文言
  → 学年（例:「2年」、省略時は1年）を読み取り、その学年の `cond_top` TOP5を返す。
  **サイトの「条件」チップと同じ7つ**（2026-09-23 きむら依頼）。語彙の正本は
  `score.py` の `CONDITIONS` で、ずれると `tools/test_bot_flow.mjs` が落ちる
- 旧プリセット名（`バイト優先` など）→ 選択肢には出さないが、打たれたら
  `preset_top` から返す。過去のトーク履歴に残っているボタンも同じ扱い
- 学年・学部を覚えている人（`line_profiles` に行がある人）は、おすすめを押すと
  学年・学部を聞かずに条件7つを出す（`knownProfileMessage`）。**選択肢を変える
  ときはここも直すこと** ―― 2026-09-23 に初回の問診だけ直して忘れ、
  常連にだけ古い選択肢が出た
...
`cond_top` / `preset_top` / `courses` は `web/data/courses.built.json` を **`env.ASSETS` 経由で
