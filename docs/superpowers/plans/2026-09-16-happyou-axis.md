...
- [ ] **Step 4b: チップとスライダーの対応表も直す（2026-09-16 実装中に判明）**

`tools/test_haiten_filter.py` の ⑤ に `CHIP_TO_CAPS` という、チップと上限の対応を
**わざと別に書き下した表**がある（`server.CHIP_CAPS` から作ると恒真になるので作らない）。
ここを直さないと「チップ『レポートのみ』とスライダーが食い違う（スライダーだけ210件）」で落ちる。
