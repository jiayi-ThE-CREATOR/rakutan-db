...
## 2026-08-26 ｜ 書いた人の「いまの学年」を画面まで通した ｜ wang → 次の人

ブランチ `feat/wang-review-grade`。

### 1. 何が動く状態か

**口コミの1件ずつに「書いた人：3年」が出る。**
`/kuchikomi` で集めていた「いまの学年」は、それまでスプレッドシートに入るだけで
サイトには一切出ていなかった（経路が5か所とも繋がっていなかった）。

```bash
python3 tools/test_reviews.py     # grade の取り込み・公開形・集計非混入
python3 server.py                 # → 科目を開く →「口コミを見る」
```
...
| # | 場所 | やったこと |
|---|---|---|
| 1 | シートの `grade` 列 | **未確認**（下記） |
| 2 | `tools/ingest_reviews.py` | `normalize()` が `grade` を読む。**白名単つき**（1年〜6年/修士/博士 以外は None） |
| 3 | `data/reviews.json` | `grade` を持つ |
| 4 | `reviews.py` `public_rows()` | 公開形に `grade` を足した。**`aggregate()` は見ない＝採点に効かない** |
| 5 | `build.py` → `reviews.built.json` → `app.js` | `panelEntry` が `書いた人：3年` を出す（`.pWho`）|

**なぜ「書いた人」と書くか**：これは受講時ではなく**書いた時点**の学年。
「3年生が回答」と書くと、3年前の口コミでも「いまも3年」と読めてしまう。

### 2. 何をしていないか

- ★**しゅんやさんのシートに `grade` 列があるか確認していない。** GAS スクリプトを
  こちらから読めないため。**列が無ければ、この機能は何も表示しないまま**
  （`grade` は None のまま流れ、画面には何も出ない ＝ 壊れはしない）。
  **最初にやること：シートの書き出し1行をもらって `grade` 列の有無を見る**
- **既存144件の口コミには `grade` が無い。** 2026-08-26 より前に集めたもので、
  当時フォームに設問が無かった。無い行には何も表示しない
  （「不明」と書くと古い口コミ全部に無意味な行が付く）
- **`server.py` の `do_POST`（サイト内の隠し投稿口）は `grade` を集めていない。**
  CAN_POST=false で塞がったままなので追随させていない。開けるときは設問を足すこと
  （docstring に書いてある）
- **集計・採点には一切入れていない。** `aggregate()` は `grade` を見ない。
  `tools/test_reviews.py` がそれを検算している
...
# シートの書き出し（TSV）を1つもらって:
python3 tools/ingest_reviews.py <export.tsv> --dry-run   # grade が拾えているか
```

### 4. 踏んだ罠

- **`tools/test_reviews.py` に「公開形のキーは固定」という検査がある。**
  公開形に足す＝公開リポジトリの `reviews.built.json` にその値が載る、なので
  一度立ち止まらせる仕掛け。狙いどおり落ちたので、期待値を更新して通した。
  **公開形に足すときは必ずここも直す**
- `?c=<科目id>` で開いた直後は詳細パネルの中身がまだ無い。動作確認は
  検索窓から科目を開くほうが確実（`?c=` は共有リンク用で、描画の順序が違う）
- `tools/test_tokens.py` は**このブランチと関係なく main で落ちている**
  （`app.css` に SNS のブランド色が裸で入った）。別途 `fix/test-tokens-brand-colors`
  が動いている。ここで足した `.pWho` は `var(--soft)` を使っていて新たな違反は無い
