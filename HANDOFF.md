...
条件チップに **「口コミあり」** が増えました。現在 **24件**。API・静的サイトの
どちらでも同じ数字が出ます。
...
- 並び順は既存チップの後ろに足しただけです。「口コミあり」を先頭に出すべきか、
  他の条件（科目の属性）とは別の行に分けるべきかは決めていません。
...
- **`CONDITIONS` は `server.py` と `web/index.html` の2箇所にあります。**
  ...
  コメントにもその旨が書いてあります。
- **チップ自体は `META.conditions` から自動生成されるので、HTML/CSS には
  手を入れる必要がありません**（`#conds` は空の `<div>` で、中身は JS が描く）。
  条件を1つ足すのに必要なのは上の2箇所の1行ずつだけです。
