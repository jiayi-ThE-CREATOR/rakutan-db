...
- **目盛りは 5 刻み（当初は 10 刻みで実装していた）。** 実データを数えたら
  配点の 7.1%（のべ1,100個）が「5の倍数だが10の倍数でない」値で、
  10の倍数でない配点を持つ科目は **708件＝9.0%** ありました。
  効くのは低いほうで、出席の上限 15% は 10% より **+134件**、25% で +108件
  （35%以上は +42/+16/+7… とほぼ効きません）。
  5の倍数ですらない値も 1.5% ありますが（33/34/66/67% ＝ 1/3・2/3 の分け方）、
  5刻みなら 33% は上限35%で拾えるので 1刻み（101段）にはしていません。
  **刻みは `app.js` の `CAP_STEP` が正本。** URL復元の丸めに 10 を直書きしていて、
  35% が往復で 40% に化けました（`tools/test_haiten_ui.mjs` が捕まえた）
