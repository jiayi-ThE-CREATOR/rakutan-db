...
## 2026-09-11 ｜ 外国語学部・日本語専攻に「言語」の二段目セレクタを追加 ｜ Claude → 次の人（wang承認待ち）

wangから：外国語学部は専攻語ごとに一つ別の言語を学ぶ（例：日本語専攻の人も中国語を履修する）。
「日本語専攻」を選んだときだけ、専攻語セレクタの隣に半分幅の「言語」セレクタを出し、
...
学部＝外国語学部 → 専攻語＝日本語 を選ぶと、専攻語セレクタの右に「言語を選ぶ」が
...
- **まだ push していない。ブランチ `feat/wang-fs-lang2` はローカルのみ**
  （PR を出すと Discord に流れるので、wang の確認後に push する）。
...
wang の確認が取れたら：
    ...
    cd .worktrees/fs-lang2
    git add -A && git commit -m "..."
    git push -u origin feat/wang-fs-lang2
    gh pr create ...

確認が要るのは「言語」の選択肢に日本語以外の24言語を単純に全部出す設計でいいか、
という1点（wangの説明どおりに実装したが、実際の学部規程の除外有無は未確認）。
