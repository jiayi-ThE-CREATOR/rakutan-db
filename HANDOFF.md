...
## 2026-08-29 ｜ ダークモードの文字コントラスト修正＋ページャーのクラス名衝突 ｜ 松下(Claude) → 次の人

wangからの依頼（8/28 23:37「右の文字が見にくい」＋添付画像）を起点に、同じ原因の箇所をまとめて直した。
ブランチは新規 `fix/darkmode-contrast`（`fix/mypage-fav-term-note` はPR #83で既にマージ済みのため、
`origin/main` から切り直した）。
...
- ダークモード全体を1画面ずつ総ざらいしたわけではない。今回直したのは
  「お気に入り一覧」「空きコマ配置ポップアップ」「カレンダー連携ポップアップ」
  「プロフィール」「初回訪問モーダル」「科目一覧のページャー」の6箇所のみ。
  他の画面（口コミフォーム・詳細パネルなど）は見ていない
