...
### 2. 何が動く状態か
    ...
    python3 tools/rebucket.py && python3 build.py --rescore   # 実行済み・コミットに含む
    tools/test_*.py 19本すべて通過
    tools/test_*.mjs：main と同じ 40/41（唯一の NG は環境要因）

    band: 情報不足 4,121 → 4,157（+36）  拘束は軽い 2,604 → 2,573（-31）  標準 443 → 438（-5）
          重め・やや重め・軽め・判定不可・参考値 は増減 0
    overall: 情報不足へ落ちた 38 / 数字が残って動いた 12（重く4・軽く8、いずれも±6以内）
             逆に数字が付いた 2（＜兼修＞フランス語初級a/b ―― 内訳が100%に届いた）

**「情報不足」が増えるのが正しい方向。** 試験だと思っていたものがレポートだった以上、
本数・分量が分かるまで数字を置かない。
...
### 1. 何が動く状態か
    ...
    tools/test_*.py 19本すべて通過
    tools/test_eval_raw.mjs：静的88件 / API86件 すべて通過（新規・シラバスとの一致を見る）
