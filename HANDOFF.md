...
`new` が入っているので MINOR（1.2 → 1.3）。PR #169、merge 後に本番反映ずみ。

### 1. 何が動く状態か

    node tools/test_version.mjs                                          # 516件 通過
    python3 tools/serve.py 8811 &
    node tools/test_version_links.mjs http://127.0.0.1:8811              # ✓31 / ✗0 / —1
    node tools/test_version_links.mjs https://rakuhan.nocode-sol.co.jp   # ✓31 / ✗0 / —1（本番でも確認ずみ）

- 主打3件（本人判断 2026-09-23）＝ ★score 重さの出し方を作り直しました ／ ★search 授業内容でさがせます ／
  ★filter つまみは「好みの重さ」に
- 口コミ1件目から・発表の独立軸・「相性度」への改名は、同じ「採点の作り直し」の話なので畳みの中へ（主打は3件まで）
- 9/21 に「10/07 の初回版まで貯める」としていた**「卒業要件」のボタンも入れた** ―― 先に版を切ることになり、
  貯める理由（空の版を出さない）が消えたため（本人判断 2026-09-23）
- `docs/version-pending.md` は空にし、「直近に出した版」の節に v1.3 / v1.2 を1行ずつ残した

### 2. 何をしていないか

- **Instagram の告知はまだ出していない。** SNS 担当に渡す原稿（カルーセル7枚＋キャプション＋NG集）は
  別途用意ずみ。Discord への共有文も未送信 ―― 授業内容タグを 9/17 に出してから一度も告知していないので、
  その分もこの版でまとめて知らせることになる
- `build.py` は流していない（`version.js` はアセットなので不要）
- 次の定例は **10/07（水）**。9月のあいだは「重要なものが溜まった時点で出す」のままで、
  10月から毎週水曜。`version-pending.md` はいま空なので、次に本番へ出す人がそこに1行足すところから

### 3. 次の人が最初に打つコマンド

    cd ~/Developer/rakutan-db && git pull
    node tools/test_version.mjs
    curl -s https://rakuhan.nocode-sol.co.jp/assets/version.js | grep 'version: "1.3"'

### 4. 踏んだ罠

- **`tools/serve.py <port>` の port が他のセッションに使われていることがある。** 8793 が埋まっていて
  `Address already in use` → 別ポート（8811）に逃がした。404 が返るときは、自分のサーバではなく
  他人のサーバを叩いている
- **時間割のコマのリンクは `/mypage`。** `#mpTimetable` を付けない（mypage.js が中身を作るので
  ブラウザが HTML を読んだ時点で id が無い ―― 9/22 に本番で実測した罠）。
  pending には古い `/mypage#mpTimetable` が残っていたので、移すときに直した
- **主打は「今回いちばん変わったこと」を3件に絞る門。** pending の ★ は 9/18 時点の仮置きで、
  そのあと本番に出た 授業内容タグ・好みの重さ が畳みに隠れる並びになっていた。
  移す前に ★ を付け直すこと

---

## 2026-09-22（3）｜ 更新履歴の「見てみる →」を本番で直した ｜ Claude（wang） → 次の人

#161 で入れた「見てみる →」は、**ローカルでは24件すべて通るのに本番では3件壊れていた**。
原因は全部ひとつ ―― **ローカルが速すぎて、本番の「データが届く前の数秒」を一度も再現していなかった**。

### 1. 何が動く状態か

    node tools/test_version.mjs                                # 363件通過（文法チェック込み）
    python3 tools/serve.py 8793 &
    node tools/test_version_links.mjs http://127.0.0.1:8793    # ✓23 / ✗0 / —1
    node tools/test_version_links.mjs https://rakuhan.nocode-sol.co.jp   # 本番にも向けられる

**`tools/test_version_links.mjs` は科目データを 2.5秒わざと遅らせてから押す。**
遅いほうだけ試せば速いほうは自動的に通る。ここを速いままにすると、また本番で踏む。

直した3件（#166 と #167）:

1. **回線が遅いと最初の数秒だけリンクが消える** ―― `#sliders` `#list` `#grid` は
   静的な HTML に在るが、app.js がデータを入れるまで高さ 0。リンクの表示を
   「大きさがあるか」で決めていた。`checkVisibility()`（CSS で消されているか）に変更
2. **別ページの `#…` がそもそも飛んでいなかった** ―― mypage.js が中身を作るので、
   ブラウザが HTML を読んだ時点で id が無い。`#mpTimetable` は上のほうに在るので
   偶然それらしく見えていただけ。マイページ向けは `#` を外した
   （`/about#strength` は静的な id なので残す）
3. **飛んだ先が、飛んだあとに伸びて画面外へ逃げる** ―― ResizeObserver で
   ページの高さを見て運び直す。利用者が自分で動かしたら手を離す

### 2. 何をしていないか

- マイページの `#…` は**直さずに外した**。精度が要るなら直す場所は `mypage.js`
  （描き終わったあとに `location.hash` をもう一度当てる）。担当が違うので触っていない
- `tools/test_version_links.mjs` は 2.5秒 × 24通りで**1回3〜4分かかる**。
  CI には入れていない（版を出す前に人が流す前提）
- 本番での最終確認は #167 のデプロイ後にまだ流していない
...
### 3. 次の人が最初に打つコマンド
    ...
    node tools/test_version.mjs
    python3 tools/serve.py 8793 &
    node tools/test_version_links.mjs http://127.0.0.1:8793

### 4. 踏んだ罠

- **「◯秒だけ見張る」は効かない。** 最初 1.6秒の見張りを入れたが、データが届くのは
  2.5秒後で、見張りが終わったあとに押し出されていた。見るべきは時間ではなく
  **レイアウトが変わったかどうか**（ResizeObserver）
- **`"scroll"` で「利用者が動かした」を判定できない。** 自分の `scrollIntoView` でも鳴る。
  `wheel` / `touchstart` / `keydown` を使う
- **`git checkout <file>` を「さっきの実験を戻す」代わりに使わない。**
  同じファイルの未コミットの直しごと消える（今日1回やった）
- **`version.js` は1文字壊れると入口ごと消えるのに、テストが素通りしていた。**
  `tools/test_version.mjs` に `new Function(src)` の文法チェックを足した。
  手で直したら `node --check web/assets/version.js` を先に通す
- **ローカルの速さを信用しない。** 24件全部 ✓ でも本番で3件壊れていた。
  版を出す前に、本番URLを引数にして同じテストを流す
...
## 2026-09-18 ｜ 採点を「相性度」に作り直した（採点の作り直しの2段目）｜ Claude → 次の人

設計 `docs/superpowers/specs/2026-09-16-aishoudo-engine-design.md`、手順
`docs/superpowers/plans/2026-09-17-aishoudo-engine.md`。ブランチ `feat/aishoudo-engine`（**未マージ**）。

### 1. 何が動く状態か

    python3 tools/test_aishoudo.py       # 式・体感層・needs_review・プリセット（40件）
    python3 tools/test_scoring_gate.py   # 口コミの取り分と、事実の穴埋めの門
    cd web && python3 -m http.server 8785 &
    node tools/test_favorite.mjs http://localhost:8785

- 相性度 ＝ (1−s)×事実層 ＋ s×体感層、事実層 ＝ 0.50×試験の楽さ ＋ 0.50×試験以外の楽さ
- 試験以外は**その科目にある項目だけ**で平均（発表.20／レポート.15／出席.15／小テスト.10）
- 試験のみ 70.0 ＜ 出席のみ・発表のみ 72.5 ＜ レポートのみ 77.5（方針どおりの並び）
- 口コミは1人目から体感層に入る（1人 6.7%／2人 13.3%／3人以上 20%）。
  持ち込み可否・レポート字数の穴埋めだけは3人のまま（`reviews.MIN_FOR_BACKFILL`）
- band 閾値 84/71/65。軽め 1,878 ／ 標準 2,022 ／ やや重め 1,305 ／ 重め 2,188 ／
  情報不足 272 ／ 判定不可 152 ／ 参考値 89
- カードの数字の名前は「相性度」
- Python 全件 ok、JS 28/29（NG は main でも落ちる test_kuchikomi_modal.mjs）

### 2. 何をしていないか

- **本番と比べてほぼ全科目の band が変わり、LINE の preset_top もほぼ入れ替わる。**
  版のお知らせで説明が要る（`docs/version-pending.md` に載せるかは wang に確認）
- 好みを調整する UI（3段目）。いまの重みは多数派の1通りだけ
...
- **全項目に固定の重みを配ると、無い項目の満点が混ざって試験なしの科目が6点幅に潰れる。**
  係数を急にしても平行移動するだけ。「ある項目だけで正規化」で 15.5点幅に広がった
- **試験の固定罰点 40 は 38〜42 の平地の真ん中。** 35 にすると最大 band 43.1% に崩れる
- 口コミの門は2つの役割を兼ねていた（採点に効かせるか／シラバスの事実を埋めるか）。
  前者だけ外した。後者を1人で許すと「口コミは最大2割」を迂回する
- **`.head` の中央には時間割コードのコピー（.ccC）が来ることがある。** そこを押すと
  コピーが動いて詳細が開かない。カードの高さは科目名や教員名の長さで決まるので、
  **並び順が変わるとどの科目で起きるかが変わる**。tools/test_favorite.mjs は
  押す先を `.head .title` に寄せた
- カードの右の列（.fit）の幅は中の小さい字が決めていた。「楽単スコア」→「相性度」で
  狭くなり、app.css が前提にしている「右の58px列」が崩れるので min-width を足した

---

## 2026-09-17（2）｜ about「配点でしぼる」の説明を今の振り分けに合わせた ｜ Claude → 次の人

### 1. 何が動く状態か

    python3 -c "import sys; sys.path.insert(0,'.'); from scrape.parse import bucket_of; print([bucket_of(x) for x in ['平常点','達成度チェック','個人のレポートとプレゼンテーション']])"
    # → ['attendance', 'exam', 'report']

about の「配点でしぼる」に、振り分けの説明が古いまま残っていたので直した。

- 「発表 30% はレポートに」→ **誤り**。2026-09-16（#149）から発表は `presentation` に入る
- 「期末レポート 40% は試験に」→ **誤り**。2026-09-07 に試験のルールから裸の「期末」「中間」を外したのでレポートに入る
- 「この4つに寄せて」「4本の上限」→ 軸は5本（`score.CAP_AXES`）
- 例は `scrape/parse.py` の `bucket_of` で実際に確かめたものに差し替えた（平常点→出席／達成度チェック→試験／同居項目→レポート）

### 2. 何をしていないか

- 版には載せない判定（⑧文言の修正）で PR を出した。merge は本人確認待ち
- 「5つの軸」の小テストの説明（毎回の小テスト・リアクションペーパー）は `score._quiz_load` の weekly_quiz と合っているので触っていない。
  ただしスライダー（配点の割合）では「リアクションペーパー」は出席に入る ―― 重さの軸と配点のしぼり込みで数え方が違う点は、about には書いていない

### 3. 次の人が最初に打つコマンド

    cd web && python3 -m http.server 8241 &
    node tools/test_index_gate.mjs http://127.0.0.1:8241

### 4. 踏んだ罠

- **振り分けを変えたら about の例も変える。** 9/07（期末・中間）と 9/16（発表）の2回、コードだけ変わって about が置いていかれた。
  例を書くときは推測で書かず `bucket_of` に通してから書く
...
### 1. 何が動く状態か
    ...
    python3 tools/subject_survey.py      # 「✓ 3つとも合格」

直前の項で「していない」と書いた2件を、wang の判断で片付けた。

- **`スポーツ・身体` をゲート③の例外にした**（`FACULTY_EXEMPT`）。169件中154件（91%）が全学教育推進機構だが、
  中身はスマート・スポーツリテラシー77件・ヘルスリテラシー28件などの体育実技で、全学でしか開講しない仕組みから来る偏り。
  学部名から作ったタグではない。例外でも割合は表示し続け、理由の無い例外は assert で落ちる
- **本文が同じなのに同数で割れていた33組・77件を揃えた**（書き換え40件）。英語の授業22組は共通部分
  （定型文の「社会問題、言語文化…」や教材のニュースは内容に数えない）、それ以外11組は和集合（題材・方法も含める）

### 2. 何をしていないか

- `build.py` への焼き込み・`server.py`・画面（モーダル）は未着手。**ゲートは合格したので、次はここ**
- `ことば・語学` 2,954件は「広すぎ」の線（3,000）の手前。画面で絞り込みに効くかは実物で確かめる
- Discord への共有は未（wang の了承待ち）

### 3. 次の人が最初に打つコマンド

    git switch feat/naiyou-tag
    python3 tools/subject_survey.py      # 合格を確認してから build.py に subjects を焼く

### 4. 踏んだ罠

- **ゲートを通すために閾値を上げない。** `FACULTY_MAX` を 0.92 にすれば今回は通るが、他のタグの本物の偏りまで見逃す。
  例外は「タグ1つ＋理由」で置く

---

## 2026-09-16 ｜ 授業内容タグ：Opus で全量を打ち直した（上限5・品質ゲートは残り1件）｜ Claude → 次の人

### 1. 何が動く状態か

    python3 tools/subject_survey.py      # commit 済みの data/subjects.ai.tsv で3表
    python3 tools/test_subjects.py       # 120件
    python3 tools/test_subject_tag.py    # 23件

- `data/subjects.ai.tsv` を **Opus で全7,909件打ち直して commit した**。Claude Code のサブスク内で subagent を並列に回した（API 課金なし）
- やり方：`build_prompt()` の出力（＝Gemini に渡すのと同じプロンプト）を約2.2万字ずつ163ページに切り、
  subagent が1ページずつ読んで `科目コード|キー` を書く → `parse_reply` → `split_reply` → `clean()` を通して TSV にした。
  **語彙外キー0件・欠け0件**
- 結果：タグあり **7,812/7,909（98.8%）**、平均 **2.05個**、4個以上 **13%**
  （0個97／1個3,206／2個2,186／3個1,407／4個633／5個380）
- シラバス本文が**完全一致**する科目は多数派に揃えた（57件）。**同名でも本文が違う班は揃えていない**
  ―― 総合英語の各班は中身でタグが違うのが正しい
- 品質ゲート（全量）：
  - ① 頻度：**全31語が50件以上で合格**（前回の死にタグ `脳・神経` 37→75）。3,000件超なし（最大は `ことば・語学` 2,954）
  - ② 共起：`歴史 ∩ 文化・地域` 531、`国際 ∩ 政治・法` 249
  - ③ 学部集中度：**`スポーツ・身体` だけ不合格**（全学教育推進機構に91%・169件）。前回不合格の `宗教` は184件で通過

### 2. 何をしていないか

- **`スポーツ・身体` の偽タグ判定はそのまま。** 体育実技は構造的に全学教育推進機構にしか無いので「学部の言い換え」ではない可能性が高いが、
  語彙とゲートの扱いは人が決める（wang 判断）
- **本文が同じなのに同数で割れた33組・77件は揃えていない**（多数派が無いため）。例：総合英語III が 3件 `ことば・語学` ／ 3件 `ことば・語学,国際`
- `ことば・語学` 2,954件は「広すぎ」の線（3,000）に近い
- **Opus の付け方は `tools/subject_tag.py`（Gemini）では再現できない。** 流し直すと Gemini の少なめの付け方に戻る。TSV が正本
- `build.py` への焼き込み・画面は未着手
- Discord への共有は未（wang の了承待ち）

### 3. 次の人が最初に打つコマンド

    git switch feat/naiyou-tag
    python3 tools/subject_survey.py      # スポーツ・身体 の扱いを決めてから build.py へ

### 4. 踏んだ罠

- 🚨 **Opus の subagent を並列で回すとサブスクの session limit に当たる。** 9本同時で約10分、5本同時でも数時間で
  `You've hit your session limit · resets …`（429）。上限は同時数より**総量**で効く
- **失敗通知が来ても、出力は書けていることが多い。** 失敗6本のうち5本は全ページ書き終えていた。
  ページ単位で書き出させ、**進捗はファイルの有無で数える**（通知を信じない）
- **同名科目の多数決で揃えると、中身で分けた正しいタグを潰す**（総合英語129班）。揃えるのは本文が完全一致した科目だけ
- subagent ごとに判断がずれる。例：英語で文献を読むだけの授業に `ことば・語学` を付けるか ―― 付けない、に統一（`海外文献調査` 84件を修正）。
  **口径はプロンプトに house rule として書いて全員に渡す**
- subagent は「ページと同名の .tsv」を `r_p012.txt.tsv` と解釈することがある。出力ファイル名は具体例で指定する
- Claude の日本語はおよそ1.1字＝1トークン。Read の上限（2.5万トークン）に収めるには1ページ2.2万字以下

---

## 2026-09-15 ｜ 授業内容タグ：全量を1回流した（打タグの bug 2件・上限3→5・流し直すモデルは判断待ち）｜ Claude → 次の人

### 1. 何が動く状態か

    python3 tools/test_subjects.py        # 120件
    python3 tools/test_subject_tag.py     # 23件
    python3 tools/test_syllabus_text.py   # 75件
    python3 tools/subject_survey.py       # data/subjects.ai.tsv を持っている人だけ。3つの表

- **政岡さんが 9/8 14:50 に生HTML 7,909件を Drive で渡してくれていた**（#一般）。9/10 まで誰も拾っておらず、
  9/9 のこのファイルの項も「政岡さんへの依頼はまだ意味がある」と書いたままだった。
  いまは wang の手元の `data/raw/`（所属別14ディレクトリ）と `data/courses.json`（7,906件）に展開済み。どちらも gitignore
- `data/syllabus_text.jsonl.gz` を全量で作った。7,909件・1.6MB。`courses.built.json` と **7,906/7,906 一致**
  （英字入りの時間割コード324件を含む。#113 が効いていることの実証）
- **全量を1回流した**（`gemini-3.5-flash-lite`、上限3のとき）。7,908/7,909 処理、サイト掲載 7,906件中 **タグあり 7,731（97.8%）**。
  判定不能175件はほぼ 卒業演習／卒業研究（本文40字前後の定型文なので、付かなくて正しい）
- 品質ゲート（全量）は **2件不合格**：
  - `脳・神経` 37件 ―― 50件未満＝死にタグ。多くは正しい（脳神経科学入門・感覚生理学）が 外国語教育(E)(H) のような誤りも混ざる
  - `宗教` 外国語学部に90% ―― 偽タグ判定。82件中74件が地域研究の専攻科目（インドネシア文化概論 等）。学部外の仏教学講義などはむしろ正確
  - `スポーツ・身体` は89%ですれすれ通過（ただし 総合英語 4件に付いている）
- この PR に足したコミット：`dc1171d`（打タグの bug 2件、下の罠）／`31e038f`（上限 3→5）／main 追従

### 2. 何をしていないか

- **上限5で流し直していない。** wang の判断で上限を 3→5 にしたが、`gemini-3.5-flash-lite` は上限を上げてもほとんど4個以上を付けない。
  同じ311件で比べると：

      上限3                                   平均1.56  4個以上 0件
      上限5（文言そのまま）                     平均1.71  4個以上 0件
      上限5＋「当てはまる限りすべて」            平均1.74  4個以上 10件（3%）
      上限5＋それに「題材・時代・地域・方法も」  平均1.88  4個以上 12件（4%）

  付く数を増やすならプロンプトではなくモデルを替えるしかなさそうで、**どのモデルで流し直すかは wang 判断待ち**
  - `gemini-2.5-flash`：同じ200件・上限3で平均2.50（63%が上限に到達）。ただし下の罠のとおり常駐 agent と枠を共有する
  - `gemini-3.5-flash`／`gemini-3.6-flash`：225件ずつのリクエストは 503。3.5-flash は50件ずつなら通るが1回246秒で、全量は現実的でない
- **語彙（`宗教`・`脳・神経` の扱い）は触っていない。** 語彙は人が決める
- **`data/subjects.ai.tsv` は commit していない。** 手元にあるのは上限3の版で、流し直すと差し替わるため。
  9/4 の項のとおり、最終版は commit する（id・科目名・タグだけで原文は含まない）
- `hosoku.json`（政岡さん 9/8 添付・成績評価補足情報 7,909件）はまだ使っていない。`data/raw/` が揃ったので
  `python3 scrape/parse.py` で `eval_note` を全件に伸ばせる（#129 は137件だけ）。**流したら `scrape/years.py` も必ず流す**
- `build.py` への焼き込み・画面は未着手（ゲートに合格してから）

### 3. 次の人が最初に打つコマンド

    git switch feat/naiyou-tag
    # data/raw/ を持っている人だけ。所属別に14ディレクトリあるので全部並べる（zsh は配列で）
    args=(); for d in data/raw/detail data/raw/*/detail; do args+=(--raw "$d"); done
    python3 tools/extract_syllabus_text.py "${args[@]}"
    export GEMINI_API_KEY=...
    python3 tools/subject_tag.py --model <決まったモデル> --size 450 --sleep 65   # 止めても再開できる
    python3 tools/subject_survey.py

### 4. 踏んだ罠

- 🚨 **打タグの件数が合っていても、中身が空のことがある。** `parse_reply` が `isdigit()` で id を絞っていて、
  英字入りの時間割コード **324件**（`00Z008` 形式。外国語197・文53・医36…）が全部捨てられていた。
  #113 で抽出側を直したのと同じ前提が、打タグ側に残っていた（`dc1171d` で修正）
- **返事に無かった科目が「タグ0個・済み」になっていた。** `got.get(id, [])` で埋めていたので、モデルが行を落とすと
  空のまま TSV に入り、`already_done()` が済みに数えて二度と投げ直さない。今は `split_reply()` で持ち越す
- **無タグが塊で出ても「モデルが崩れた」と決めつけない。** 450件ずつで1バッチ247件が空になり、一度そう誤認した。
  実際は上の `isdigit()` で、英字 id が所属ごとにファイル内で固まっていただけ。モデル側の取りこぼしは約2.6%
- 🚨 **`gemini-2.5-flash` で一括処理しない。** 別リポジトリ（ClaudeCode）の常駐 agent ―― daily_brief / financial_news /
  shukatsu_distill_auto / paper_reader ほか ―― が全部このモデルで、無料枠は「モデルごと・プロジェクトごと・1日20回」を
  共有する。9/11 に打タグで焼き切り、翌 9/12 08:34 に daily_brief が 429 を食った（run.sh の再試行で復旧）
- **429 が成功と交互に出るのは分次の制限。** 1回450件≒11万トークンを4秒間隔で投げると交互に落ちた。
  `--sleep 65` で出なくなった。日次か分次かは、小さいリクエストを1回投げて 200 が返るかで見分ける
- **モデル一覧 API に載っていても呼べるとは限らない。** `gemini-2.5-flash-lite`・`gemini-2.0-flash` は
  「新規ユーザーには提供終了」で 404
- **zsh では `$ARGS` が単語分割されない。** `--raw` を複数並べるときは配列で `args+=(--raw "$d")`

---

## 2026-09-04 ｜ 授業内容タグの語彙と打タグ・品質ゲート（本文待ち）｜ Claude → 次の人

### 1. 何が動く状態か

    python3 tools/test_subjects.py       # 120件（語彙とルール）
    python3 tools/test_subject_tag.py    # 19件（バッチ・再開・返事の読み取り）
    python3 tools/subject_survey.py      # 3つの品質ゲート

3本入れました。`tools/subjects.py`（語彙31語・科目名ルール・出所の優先順位）、
`tools/subject_tag.py`（Gemini へバッチ投げ）、`tools/subject_survey.py`（品質ゲート）。

**手元の実データ10件で Gemini まで通してあります。** 設計の中心にあるケース、
`ことばの学問入門`（科目名に「歴史」は出てこない）に本文を渡すと
**`ことば・語学 ＋ 歴史`** が付きました。科目名だけなら `ことば・語学` だけです。

### 2. 何をしていないか

- **全量を流していません。** `data/syllabus_text.jsonl.gz` が要ります
  ―― 政岡さんが `tools/extract_syllabus_text.py` を流して渡してくれるのを待ち中
- **語彙31語はまだ草案です。** 品質ゲート（頻度・共起・学部集中度）は
  **7,906件そろって初めて意味を持ちます**。手元10件では当然すべて不合格になります
  （ゲートが動いていることの確認にはなります）
- `build.py` への焼き込みと画面（モーダル）はまだです。ゲートを通してから
- `data/subjects.ai.tsv` は gitignore していません。中身は id・科目名・タグだけで
  シラバス原文を含まないので、**全量を流したら commit してください**
  （「数字の根拠を全部提示できる」ため）

### 3. 次の人が最初に打つコマンド

    # 政岡さんから data/syllabus_text.jsonl.gz を受け取ってから
    export GEMINI_API_KEY=...
    python3 tools/subject_tag.py --limit 200     # まず200件で様子を見る
    python3 tools/subject_survey.py              # 3つの表を見る
    python3 tools/subject_tag.py                 # 良さそうなら全件（途中で止めて再開可）

### 4. 踏んだ罠

- **macOS の python.org 版 Python は TLS の証明書を検証できません**
  （`CERTIFICATE_VERIFY_FAILED`）。`/usr/bin/python3` なら通ります。
  `subject_tag.py` は 既定 → certifi の順に試し、どちらも駄目なら直し方を出して
  止まります。**検証を切る選択肢は置いていません**（鍵を載せた通信なので）
- **本文が「英文シラバスをご参照ください。」の1文だけ、という科目があります。**
  最初のプロンプトでは `Cross Cultural Psychology` に何も付きませんでした。
  「概要が空なら科目名から確実に言えるものだけ付ける（想像で補わない）」を
  足して直っています
- **モデルはやや多めに付けます**（`ことばの学問入門` に `生き物` が付く ――
  第1章が動物のコミュニケーションなので誤りではない）。多すぎるかは10件では
  決められないので、頻度表を全量で見てから調整すること
- **プロンプトに語彙を書き写さないこと。** `tools/subjects.py` の `vocab_prompt()`
  が唯一の定義です。書き写すと語彙を足したときにプロンプトだけ古くなり、
  モデルが知らないキーを返して `clean()` に黙って落とされます
  （＝タグが付かない科目が増えるだけで、原因が画面から見えない）
- 設計スペックに「語彙33語」と書いていましたが**数え間違いで実際は31語**でした。
  正本は `tools/subjects.py` の `VOCAB` です
...
手順は `docs/superpowers/plans/2026-09-16-happyou-axis.md`。ブランチ `feat/happyou-axis`（**未マージ**）。

### 1. 何が動く状態か

    python3 tools/test_happyou_axis.py        # 振り分け・発表軸・保底・上限・チップ（40件）
...
- band 閾値 83/77/69。やや重め 2,322 ／ 重め 2,192 ／ 拘束は軽い 1,688 ／ 標準 1,190 ／ 情報不足 272
- 配点スライダーに「発表」、条件チップに「発表なし」（5,004件）。「レポートのみ」は 508 → 298
- LINE の `preset_top` は6組とも不変（実測）
- 9/16 時点の main（#139 LINE ゲート・#145 リセット を含む）を取り込み済み。Python 全件 ok、
  JS 27/28（NG は main でも落ちる `test_kuchikomi_modal.mjs` の `.rv` のみ）。
  リセットは `CAP_AXES` をループするので発表のスライダーも戻る
...
- #139 の LINE ゲートで、未登録の人には「発表なし」チップも発表スライダーも「見えるが押せない」
...
- **テストの中に軸キーの写しが3つあった。** `test_haiten_filter.py` の `NO_CAP`（→ `CAP_AXES` から
  作るよう直した）、`caps_impossible` の辞書（`_cap` は無いキーを 100 と数えるので合計が 180 に
  なった）、`CHIP_TO_CAPS`（**わざと写し**。`server.CHIP_CAPS` から作ると恒真になるので写しのまま直した）
...
- `tools/test_haiten_ui.mjs` の「合計が100%を下回ると警告」は本数×1本の値で決まる。
...
`#127` が画面を直しても、`環境評価論`（`期末レポート 100%`）は**スコアの上では
まだ試験100%**で、条件チップ「レポートのみ」にも出てこない状態だった。

### 1. 何が問題だったか

`METHOD_RULES` の試験ルールが `試験|テスト|筆記|期末|中間|口頭試問|exam` で、
**裸の「期末」「中間」が入っていた**。この2語は評価の**形**ではなく**時期**しか
言っていないのに、レポートのルールより先に当たる。

    期末課題30 / 中間課題21 / 期末レポート14 / 中間レポート10 / 中間発表7 / 期末エッセイ4 ……
    延べ172箇所が試験に化けていた（全7,906件で実測）

**ズレの向きは常に「実際より軽く見える」側**だった。試験軸は eval_ratio の比率だけで
値を出せるが、レポート軸は本数・分量が無ければ None を返す（`_report_load`）。
つまり**測れないものに数字が付いていた**。`【社会】国際社会における法と政治` は
`学期末レポート 100%` の1項目だけで 88.8「拘束は軽い」だった。

### 2. 何が動く状態か

    python3 build.py --rescore   # eval_raw からの振り分け直しも含む。実行済み・コミットに含む
    tools/test_*.py 20本すべて通過
    tools/test_*.mjs：静的サーバ（python3 -m http.server 8744 --directory web）に向けて 24/25
      唯一の NG test_kuchikomi_modal.mjs「詳細に口コミの集計（.rv）が残っている」は
      **main でも同じく落ちる**（9/10 に .rv を復元したがテストが古いまま）。この PR とは無関係

    ── 2026-09-15、#134（レポート軸を割合ベースに）の後の main へ載せ直して測り直した ──
    band: 重め 1,981 → 2,032（+51）   やや重め 2,058 → 2,084（+26）
          標準 1,757 → 1,722（-35）   拘束は軽い 1,588 → 1,554（-34）   情報不足 280 → 272（-8）
    overall: 数字が動いた 108（重く89・軽く19）／判定が消えた 0／判定が付いた 8／band が変わった 84
    例: 環境評価論（期末レポート100%）  90.1「拘束は軽い」→ 66.5「重め」

**9/7 時点の「情報不足が増えるのが正しい方向」は #134 で前提が変わった。** いまは
レポート軸も割合から値を出すので、誤分類が直った科目は情報不足ではなく**レポートとして
採点される**。動く向きはほぼ全部「重く」で、「試験に化けて実際より軽く見えていた」という
元の指摘どおり。

🚨 **ただし「試験だけ」と「レポートだけ」の並びが、wang の方針と逆になっている。**

    試験のみ     423件  中央値 90.1「拘束は軽い」
    レポートのみ 508件  中央値 66.5「重め」

方針は「多くの学生は試験を嫌い、レポートのほうがまし」。原因はこの PR ではなく #134 で、
レポートの係数を 0.45 に上げたのに試験は 0.15 のまま。この PR で「レポートのみ」が
26件増えるので目に付きやすくなる。直すのは次の spec（試験軸に「有無の落差」を入れる）。

### 3. 外した2語の行き先 ―― ここを飛ばすと配点が消える

裸の「期末」「中間」を外すと**8箇所が未分類に落ちる**。未分類は画面にもログにも
出ず「内訳が取れていない科目」として静かに扱われるだけなので、**テストでは気づけない**。
受け止めるルールを同時に足した:

    ディベート                              → attendance   期末ディベート3・総合英語ほか10箇所
    達成度チェック|レベルチェック|アチーブメント  → exam         中間/期末達成度チェック4＋未分類だった12
    リポート                                → report       期末リポート1

**`達成度チェック` を quiz ではなく exam に入れた理由**（実測して決めた）:
語学科目の**定期の到達度試験**で、配点は25〜50%、「中間」「期末」を冠したものもある。
quiz 軸が測っているのは「毎回そのつど準備が要る」拘束（`_quiz_load`）で、
学期に数回の試験はそれではない。quiz に入れると
`フランス語初級I`（小テスト30 ＋ 中間35 ＋ 期末35）が「小テスト100%」になり
**74.4「標準」→ 31.8「重め」まで飛ぶ**。exam なら 74.4 のまま動かない。
「中間／期末」を冠した4箇所は元から exam なので、据え置きでもある。

### 4. 条件チップ ―― 3つとも増えただけで、抜けた科目は0件

    レポートのみ  482 →  508   「期末レポート」が試験から外れた
    出席なし     2170 → 2176   フランス語13(A)ほか。`レベルチェック（春夏学期）`が未分類で
                               内訳が最後まで読めず**チップから外れていた**のが解消
    小テストなし 5939 → 5953   フランス語14(A)ほか。`達成度チェック`が同じ理由

`tools/test_conditions.py` の期待値を理由つきで更新した。
**見るべきは数字が変わったことではなく「抜けた科目があるか」**（-0 かどうか）。

### 5. 何をしていないか

- **未分類はまだ 705箇所 / 約500科目。**（この PR で18箇所だけ回収した）
  中央値で**35%の配点が消えたまま**で、50%以上消えている科目が約195件ある。
  族ごとの内訳と、判断が要る2つ:
      宿題・提出物112 → report で良さそう
      授業内活動74   → attendance で良さそう
      予習・復習53   → **既存の4軸のどれでもない**（授業時間外の負担）。要判断
      実習・研究47   → 医学部・看護の実習科目。report か新概念か。要判断
      参照文65（`下記評価基準`32・`補足情報参照`・`英文シラバスをご参照ください。`8）
                    → **これは分類してはいけない**。評価成分ではなく「表がここに無い」
                      という意味なので、未分類のままが正しい挙動
      一点物357      → モンゴル語 `даалгавар`・スワヒリ語 `Kazi`・デンマーク語など
                      語学科目の原語表記が中心。長い尾
- **複合項目「その他（A、B、C）」型 延べ180箇所も手つかず。**
  最大が `その他（レポート、課題提出、小テスト、発表、学習への参加度）` **120件**で、
  「小テスト」が先に当たるので**5類ぜんぶの配点が小テストに入っている**。
  1項目を複数バケツへ按分する仕組みが要る

### 6. 罠

- **ルールから語を外すときは、外した後どこへ落ちるかを全件シミュレーションしてから外す。**
  `eval_raw` を全件なめる使い捨てスクリプトで先に見る。テストは未分類を検出しない
- **受け皿のバケツは「意味が合うか」で選ぶ。件数が合えばいいのではない。**
  `達成度チェック` を quiz に入れれば未分類は0になるが、`_quiz_load` が意味しているのは
...
## 2026-09-15 ｜ 時間割コードのコピーを「科目名の下の行」へ移す＋その行を13pxに ｜ Claude → 次の人

wangから2点。①科目名の下の行（曜限・教員・キャンパス・区分・コード）の字が小さい、
②前日に入れた「詳細の全幅コピーチップ」は要らない、コピーはコードの右に文字で置く。

### 1. 何が動く状態か

    cd .worktrees/koanmeta/web && python3 -m http.server 8206 &
    node ../tools/test_koan_code.mjs http://localhost:8206     # OK 20 checks

- `.meta` を 12px → `var(--fs-sm)`（13px）・`line-height:1.6`
- コードの右に `⧉ コピー`。押すと `✓ コピー済`（太字）に1.6秒だけ変わる
- `.dActs` の全幅チップ（`.codeChip`）は HTML も CSS も撤去

### 2. 何をしていないか

- **押せるのは「⧉ コピー」の文字だけ。数字は押せない**（押せばカードが開く）。
  行全体を押せるようにすると、一覧カードでは `.head`（role="button"）の中なので
  「カードを開こうとして数字に当たり、開かずにコピーされる」が起きる
- コピーの手応えを「コピーしました」にしていない ―― `⧉ コピー` より数文字長く、
  その1.6秒だけ押せる面が広がる。`✓ コピー済` なら幅がほぼ変わらない
- 390px では `.meta` が3行に折り返す科目がある（13px にした副作用）。
  実機では見ていない
...
- **`.head` の中央は、科目によってちょうどこの「コピー」に重なる**（1280px・
  先頭カードで実測）。`tools/test_favorite.mjs` と `test_inspector_center.mjs` が
  `.head` の中央クリックでカードを開いているので、当たり判定を広げすぎると
  この2本が落ちる ―― 落ちたら当たり判定を疑う。テスト側は直さないこと
- 委譲は **capture 段**（`addEventListener(..., true)`）。バブリングだと
  `.head` の `onclick` が先に走ってカードが開いてしまう
- `.meta` の区切り「・」は `span+span::before` だった。PCの右ペインだけ
  コピーが `<button>` になるので、`.meta>*+*::before` に変えてある
...
## 2026-09-14（つづき）｜ カルーセルの見た目調整＋ダークモード対応 ｜ Claude → 次の人

直前の項目（画像スライドショー化）に対する松下さんからの一連のフィードバックをこの1本にまとめて記録する。
やり取りの中で `/design`（Claude Design キャンバス）で3案を見せ、**案C「背景で1枚にまとめる」**を選んでもらった。

### 1. 何が動く状態か

    python -m http.server 8123 --directory web

`http://localhost:8123/about.html` の「ラクハンの強み」で確認できる変更点：

- 画像1枚ごとに **グレー地のパネル＋白い角丸カード**（`.strFig` に `background:var(--dim)` と
  `padding:var(--sp-3)`）。1〜5枚目すべて同じ地に統一（以前は画像だけがカードいっぱいに敷かれていた）
- 5枚目だけ画像2枚を横並び・半分幅ずつ（`.strFigSplit`）。2枚とも 544×720 に揃えてあるので高さが揃う
- **ダークモードでは、ダークモードで撮った別のスクショに差し替わる**（`<picture><source
  media="(prefers-color-scheme: dark)">`）。ライト版のスクショをそのまま暗い地に置くと
  「全部真っ黒」に見えたための対応。オレンジのスコア・緑の口コミ・グラフの色は暗地でも
  ちゃんと目立つことを実機で確認ずみ
- 矢印・ドットとカードの間の余白を詰めた（`--sp-5`→`--sp-3`）

変更・追加したファイル：

- `web/about.html` ― 各スライドの `<img>` を `<picture><source>...<img></picture>` に変更（ダーク版切替）。
  5枚目のキャプションを「あなたの学年・学部学科・好みから探せます」に更新
- `web/assets/about.css` ― `.strFig` のパネル地、`.strFigSplit` の並び、`.strCtrl` の余白を調整
- `web/assets/strengths/` ― 各スライドのライト版に加え `*-dark.png`（同じ構図をダークモードで撮り直したもの）を追加。
  5枚目の右側画像は「学年（1年選択）・全学部共通の区分でしぼる（閉じた状態）・学部からさがす（基礎工学部・学科を選ぶ）」
  という松下さん指定の範囲で撮り直し済み

### 2. 何をしていないか

- **PR はまだ作っていない**（このあと作る。松下さんから「本番に反映したい、PRは挟んで」と指示ずみ）
- PR 前に流す一式（`build.py` ・6本の `test_*` ・`tools/shots.mjs`）はこのあと実行する
- 版（バージョン＆最新機能）に載せるかどうかの最終確認はまだ。最初の実装時に一度「【版に載せる？】」を
  出したが、その後何度も内容が変わっているので、PR を出す前後で松下さんに改めて確認すること
- **`web/assets/app.js` に、このタスクと無関係な未コミットの変更が残っている**
  （`facH` の見出し文言を「学部からさがす」→「学部学科からさがす」に変える1行。誰の変更か・
  いつ入ったかは不明 ―― 少なくとも origin/main には無い）。今回のコミットには含めていない。
  松下さん本人か他のセッションの作業中のものである可能性があるので、**消さずにそのまま残してある**。
  気づいたら本人に確認すること
- ローカルの `main` が push 時点で **origin/main より19コミット遅れていた**ので、`main` には
  コミットせず `feat/matsushita-about-strengths-carousel` ブランチを切って作業した

### 3. 次の人が最初にやること

    python -m http.server 8123 --directory web

`about.html` の「ラクハンの強み」を、ライト・ダーク両方、5枚とも確認。ダークモードは
OS／ブラウザの配色設定をダークにするか、ブラウザの devtools で `prefers-color-scheme: dark` を
エミュレートすると見られる。

### 4. 今回踏んだ罠

- **`<span>` や `<picture>` は既定で `display:inline`。中に `display:block` の `<img>` を入れて
  `width:100%` を指定しても、その 100% の基準（containing block）は inline の親を素通りして
  もっと外側の block 祖先になる。** 上下の padding は inline のまま効くのに、左右の padding だけ
  効かない、という形で発覚した（本人指摘）。`.strFig` と `.strFig picture` の両方に
  `display:block` を足して直した。同じ罠にもう一度落ちたので、次に画像まわりのラッパー要素を
  足すときは最初から `display:block` を疑うこと
- **Playwright の `page.screenshot({clip})` は、要求した範囲がビューポートの高さを超えていると
  黙ってビューポート内に収まる分だけ撮って終わる**（エラーにならない）。ダークモード版の
  「学年・学部」画像を撮ったとき、viewport の高さを他のショットと同じ 1400px のままにしていたら
  354px 分（画像の下半分＝学部セレクタごと）が欠けた状態で保存されていた。
  縦に長い範囲を撮るときは viewport の高さを余裕を持って（今回は 2000px）確保すること
- **このセッションのブラウザツールは、同じ URL の画像を強くキャッシュする。** 同じファイル名で
  中身だけ差し替えても、既存タブはおろか「新しく開いたタブ」でも古い画像が出続けることがあった
  （`fetch(url, {cache:'no-store'})` で中身が更新されていることは確認できるのに、`<img>` 要素は
  古いまま）。本当に新しい内容を目で確認したいときは、一度も同じオリジンを開いたことのない
  真っさらなタブを使うか、`img.src` にクエリを足して強制的に再取得させること。
  ―― これは開発中の確認だけの問題で、実際の利用者のブラウザには影響しない（利用者はこの画像を
  古い版で見たことが無いので、通常のキャッシュ動作で新しい画像を取得する）

---

## 2026-09-14 ｜ About「ラクハンの強み」を画像スライドショーに（実画面のスクショ5枚）｜ Claude → 次の人

松下さんから「aboutページの『ラクハンの強み』を、参考サイト（KULMS+ の『主な機能』）みたいに
画像をスライドしながら紹介する形にしたい」との依頼。画像は「実際の画面のスクショを撮って使う」で
確認を取ってから着手した。

### 1. 何が動く状態か

    python -m http.server 8123 --directory web   # または web-static

about.html の「ラクハンの強み」が、テキスト箇条書き5行から、実際の画面のスクリーンショット5枚を
横スクロール（scroll-snap）で送れるカルーセルに変わった。矢印（‹›）とドット5個で操作、キーボードの
←→でも送れる。デスクトップ・スマホ（375px）・ダークモードで目視確認ずみ。画像は本番サイト
（https://rakuhan.nocode-sol.co.jp）を Playwright のヘッドレスブラウザで実際に撮影したもの
（4枚目「KOANへ1タップ」だけは、科目タイトルと KOAN ボタンの2枚を「…」でつないだ合成画像。
理由は下の「踏んだ罠」）。

変更・追加したファイル：

- `web/about.html` … 「ラクハンの強み」の `<ul>` を `<figure>` 5枚のカルーセルに置き換え。
  `<head>` に `about.css`、body 末尾に `about.js` を追加
- `web/assets/about.css`（新規）… カルーセルの見た目だけ。既存の `.prose h2/p` は触っていない
- `web/assets/about.js`（新規）… 矢印・ドット・←→キーの操作。scroll-snap に乗せているだけなので、
  指でのスワイプ自体はブラウザ標準の挙動
- `web/assets/strengths/`（新規、5ファイル計412KB）… 本番サイトの実スクショ
  （`all-courses.png` / `no-login.png` / `no-review-score.png` / `koan-link.png` / `faculty-filter.png`）

### 2. 何をしていないか

- `tools/shots.mjs` には追加していない ―― このカルーセルは「機能紹介用の説明画像」で、
  PR ごとに撮り直す対象（画面そのものの見た目）とは性質が違うと判断した。もし科目カードや
  詳細パネルの見た目が変わったら、この5枚は手で撮り直しが要る。撮影に使ったスクリプトは
  使い捨てで、`tools/` 配下にも保存していない（このメモの「踏んだ罠」に手順の要点だけ残す）
- 版（バージョン＆最新機能）に載せるか、松下さんにまだ確認していない
- PR を出す前に流す一式（`build.py` ・6本の `test_*` ・`tools/shots.mjs`）はまだ実行していない。
  今回の変更に関係しそうな4本（`test_tokens` / `test_web_split` / `test_layout` /
  `test_shell_inject`）だけ先に確認した ―― 全部 OK
- スクショ自体はライトモードの画面をそのまま使っている（枠の地はダーク対応済みだが、
  白い画像がダーク画面に浮く）。目視で許容範囲と判断したが、気になるならダーク版を別撮りする余地がある

### 3. 次の人が最初にやること

    python -m http.server 8123 --directory web

`http://localhost:8123/about.html` を開き、「ラクハンの強み」まで下にスクロール。矢印とドットで
5枚とも送れるか、スマホ幅（375px）とダークモードでも確認。

### 4. 今回踏んだ罠

- 本番サイトの絞り込みサイドバーに **`id="facSec"` が2箇所**ある（「全学部共通の区分でしぼる」と
  「学部からさがす」が同じ `<section id="facSec">` に同居している）。
  `document.querySelector('#facSec')` だと先に出てくる方（共通区分）を拾ってしまい、
  狙った「学部からさがす」のほうが撮れなかった。見出しのテキストで要素を特定して回避した
- Playwright の `element.boundingBox()` はページの絶対座標ではなく「今のスクロール位置から見た
  ビューポート座標」を返す。折り返し前の位置のまま画面外の要素を `page.screenshot({clip})` に渡すと
  `Clipped area is either empty or outside the resulting image` で落ちる。
  `scrollIntoViewIfNeeded()` してから座標を取り直すこと
- 詳細パネルの「成績評価の内訳〜口コミの集計」を丸ごと撮ると、KOAN ボタンへの導線が
  情報過多で埋もれた。科目タイトルの見出し部分と KOAN ボタン部分を別々に撮って、
  PIL 側で「…」を挟んで縦に合成する方式に変えた（`web/assets/strengths/koan-link.png`）
...
「日本語専攻」を選んだときだけ、専攻語セレクタの隣に半分幅のもう一つの専攻語セレクタを出し、
選んだ言語の専攻語科目（＝その言語専攻の学生と同じ科目）をそのまま出すようにした。
wang承認済み・PR経由でmain にマージ済み。

### 1. 何が動く状態か

    cd .worktrees/fs-lang2/web && python3 -m http.server 8199 &
    node ../tools/test_fs_lang2.mjs http://localhost:8199

学部＝外国語学部 → 専攻語＝日本語 を選ぶと、専攻語セレクタの右に「専攻語を選ぶ」が
半分幅で並んで出る（`.trackRow.split`）。中国語を選ぶと、直接「専攻語＝中国語」を
選んだ場合と**完全に同じ一覧**になる（テストで件数・IDとも一致を確認）。
API モード（`server.py` の `/api/courses`）にも `track2` パラメータとして同じ手順で入れた
（静的モードの `effectiveTrack()` と同じロジック）。学部を変える／専攻語を日本語以外に
変えると `state.track2` は捨てられ、セレクタも隠れる。

### 2. 何をしていないか

- 言語の選択肢は `fac.tracks`（`requirements.json` の外国語学部 tracks）から
  日本語自身（`fs_lang:R`）を除いた24件そのまま。除外リストなどは無い
  （実際の学部規程で選べない組み合わせがあるかは未確認だが、wang確認済みのため実装通り）。
- スマホ幅での見た目は目視していない（select 2つを横並びにしただけなので、
  文字が長い言語名で詰まる可能性はゼロではない）。
- `tools/check_division_ui.mjs` は今回の変更前から同じ場所でタイムアウトして落ちる
  （`git stash` して確認済み・別件）。今回のテストはこれとは別に `tools/test_fs_lang2.mjs`
  を新規で書いて通した。

### 3. 次の人が最初にやること

とくに無し。動作確認したいときは①のコマンドで。

### 4. 踏んだ罠

- `#facSec select{width:100%}` が既存であるおかげで、新しい select にスタイルを
  1行も足さずに済んだ。ラッパーに `display:contents` を使い、`.split` が付いた
  ときだけ `display:grid;grid-template-columns:1fr 1fr` にする形にすると、
  他の学部（トラック1個 or 0個）の見た目に一切影響しない。
- フィルタ側は `state.track` を直接見ている箇所（UIの選択状態表示）と、
...
## 2026-09-11 ｜ 条件チップの並びと点灯／「あなたに合う」枠の重複／リセット（保留）｜ Claude → 次の人

林さんから3件＋2件の指摘。**3件は push 済み、リセットだけ返事待ちで止めてある。**

### 1. 何が動く状態か

    cd web && python3 -m http.server 8144 &
    node tools/test_conds_layout.mjs http://localhost:8144   # 条件チップの並びと点灯
    node tools/test_picks_dedup.mjs  http://localhost:8144   # 「あなたに合う」枠の重複

ブランチ `feat/hayashi-conds-picks` → **main にマージ済み（本番に出ている）**。
途中で main が7件進んだ（#134 レポート軸の割合ベース化＋データ焼き直し）ので、
取り込んでから新しいデータで全テストを流し直してある。

- **b19c5d7 条件チップを3列×2行に固定し、含まれている側を淡く出す**
  「レポートのみ」は試験・出席・小テストの3軸を 0% にするので、1軸だけ見る
  「出席なし」「小テストなし」も同時に光っていた。押した1つは `.chip.on`、
  含まれて点いている側は `.chip.on.imp`（`--brand-soft` の淡い地）に分けた。
  **絞り込みの中身は変えていない** ―― 実際に出席0%で絞れている以上、
  消灯させると画面が嘘をつく。並びは `flex-wrap` → `grid` 3列固定。
- **6231d1e 「あなたに合う」枠に出した科目を下の一覧から外す**
  枠の3件がすぐ下の一覧にもそのまま並んでいた。**全ページから外す**。

### 2. 何をしていないか

- **リセットボタンは未コミットのまま手元にある**（`web/index.html`・`app.css`・
  `app.js` の未コミット差分と `tools/test_reset.mjs`）。動作は全部OKだが、
  スマホで `.railHead` を出す必要があり、これが huaqianshue さんの取り決め
  （`tools/test_rail_toggle.mjs:219`「スマホ幅で条件欄の見出し行が出ている」）に
  当たる。**本人の返事待ちで止めている。**
- **版に載せるかが未決**（判定②と④、林さんの回答待ち）。決まったら
  `docs/version-pending.md` か `web/assets/version.js` に入れる。
- `data/courses.json` が手元に無いので、`web/data/courses.built.json`（7,906件）
  を静的配信して確認した。`server.py` 経由の API モードは踏んでいない。

### 3. 次の人が最初に打つコマンド

    git fetch origin && git checkout feat/hayashi-conds-picks
    npm install && npx playwright install chromium   # node_modules が空だった
    cd web && python3 -m http.server 8144 &
    node tools/test_conds_layout.mjs http://localhost:8144

### 4. 踏んだ罠

- **PC では「名前＋件数」を横一列に置けない。** 条件欄はレールの中で 240〜280px
  しかなく、3で割ると1枠 79px。`font-size` を 10px まで縮めても 95px 要る。
  件数を名前の下に落として初めて3列が成立した。**目分量で決めず実測すること。**
- **「条件があるときだけリセット行を出す」は罠。** 条件を1つ押した瞬間に行が
  生まれ、下の条件チップ6個が全部ずれる。**直したばかりの「押すと配置が変わる」を
  別の形で作り直すことになる。** 行は常に置いて高さを固定した。
- **仮マージの警告は真に受けない。** 他ブランチとの衝突を見たら14本で警告が出たが、
  全部「そのブランチが元々 main とぶつかっている」既存分だった。
  `merge-tree origin/main <br>` を基準に差を取って初めて「自分が増やした分は0」と
  分かる。基準を取らずに読むと、無関係な衝突を自分のせいだと誤診する。
- **push だけでは本番に出ない。** 本番は `main` から作られる。ブランチを push した
  時点で「反映されていない」と言われたが、不具合ではなくマージ待ちだった。
  公開中の JS を `curl https://rakuhan.nocode-sol.co.jp/assets/app.js | grep <目印>`
  で見れば、出ているかどうかが1行で分かる。
- **`test_sort` と `check_division_ui` は前から落ちている。** 自分の変更のせいかは、
  変更を `git stash` して同じテストを流せば1分で切り分けられる（今回そうした）。
  `test_sort` はその後 OK になったので、データ依存で揺れる。**別件として未解決。**
- 学部の値は `engineering` などの key で、画面の「工学部」は label。
  テストに label を書くと `<select>` が既定のままになり、直っているのに落ちる。

---

## 2026-09-10 ｜ 口コミの一言を吹き出しにする（Claude Design で4案から選定）｜ Claude → 次の人

松下さんから「科目詳細欄の口コミの一言を、吹き出しみたいな感じにしたい」との依頼。
`/design` で実データ・実トークンに合わせた4案（上向き吹き出し／そっと囲む／チャット風／
強調カード）を並べたキャンバスを作って見せ、松下さんが**案2「そっと囲む」**を選んだ。

### 1. 何が動く状態か

    python server.py --port 8000        # または web-static（8123）

口コミが1件でもある科目を開くと、集計数字の下に灰色地（--dim）＋引用符（"）で
軽く囲んだ一言が出る。尻尾は無し。デスクトップ・スマホ・ダークモードいずれも
実際にブラウザで確認ずみ（値は computed style で --dim/--ink/--muted のダーク値と
一致することまで確認）。

変更したファイル：

- `web/assets/app.js` … `detailHtml()` の一言を `.pNote` ではなく新しい `.dQuote`
  で出すよう変更（`.pNote` は口コミモーダルの1件ずつ表示 `panelEntry` と共有の
  クラスなので、詳細側だけ変えるなら分けないとモーダル側の見た目も変わってしまう）
- `web/assets/app.css` … `.dQuote` を新規追加（`.rvf` のすぐ下）
- `tools/test_tokens.py` … CONTRAST に `.dQuote::before` の引用符
  （`--muted` on `--dim`、3.0）を追加。あわせて、2026-08-24 に一度実装され
  2026-09-07 に detailHtml ごと消えていた「口コミの吹き出し」（`--brand-soft` 地）用の
  古い登録2行を整理（1行は `.picksH`（推薦枠見出し）が今も使っているので改名して残し、
  もう1行は他に使い道が無いので削除）

### 2. 何をしていないか

- **Claude Design のキャンバスは公開したまま**（[Kuchikomi Bubble Options](https://claude.ai/code/artifact/00f7b6ca-fe00-41e6-8a6b-3bf2864eabfe)）。実装が終わったので消してよいが、次の人の判断に任せる
- 「そっと囲む」以外の3案（上向き吹き出し／チャット風／強調カード）はコードには残していない。キャンバス側にだけ残る
- 版（バージョン＆最新機能）に載せるかどうかは松下さんにまだ確認していない（このあと聞く）

### 3. 次の人が最初にやること

    python tools/test_tokens.py    # 115件通過（コントラスト72組）
    python tools/test_layout.py    # 21件通過

口コミ本文がある科目（例：135163 の月3・佐々木回、reviews.n=2）で `.dQuote` の見た目を確認。

### 4. 今回踏んだ罠

- **ローカルの `data/courses.json`（server.py が読む生データ）には本文入りの口コミが
  ほぼ無い。** `web/data/courses.built.json`（静的ビルド済み・build.py が焼いたもの）
  には83件ある。口コミの見た目を確認するときは `web-static`（8123）か、
  `courses.built.json` 側を見ること ―― `server-py`（8000）だけで試すと
  「本文入りの口コミが1件も無い」という誤った状態を疑うことになる
- **2026-08-24 に一度、ほぼ同じ「吹き出し」機能が実装されていた**（`--brand-soft`地・
  引用符・左下の尻尾）。2026-09-07 に口コミ節ごと detailHtml から外れて消え、
  そのとき tools/test_tokens.py の CONTRAST 登録だけ削除し忘れて残っていた
  （`git log -S"口コミの吹き出し"` で発見）。今回は同じ機能でもデザインが違う
  （dim地・尻尾なし）ので、そのまま復活はさせず、死んだ登録として整理した

---

## 2026-09-10 ｜ 科目詳細に口コミの集計を復元（wangさんの依頼、9/10 24時までの約束）｜ Claude → 次の人

Discord `#ラクハンーui＆design` 9/10 14:13、wangさんから「科目詳細に成績の内訳しか残っていない、
『口コミを読む』の部分と図2の部分も追加してほしい。口コミは一部でいいので上の数字が欲しい」との依頼。
松下さんに確認の上、「図2」＝wangさんが同じメッセージで添付した2枚目の画像（口コミモーダルの中身：
集計数字＋口コミ本文1件）と解釈して実装した。

### 1. 何が動く状態か

    python server.py --port 8000

ブラウザで科目を1件選ぶと、詳細パネル（PC=右カラム／スマホ=カード内）の
「成績評価の内訳」と「KOAN公式シラバスを見る」の間に「口コミ N件」の集計
（出席・授業中の課題・授業外の課題・テストの難易度・持ち込み・レポート）が出る。
本文がある口コミなら、その下に一言を1件だけ引用する。

変更したファイル：`web/assets/app.js`（`detailHtml()` に `reviewHtml()` 呼び出しと
本文1件の引用を復元）、`web/assets/app.css`（`.dSec+.dSec` の区切り線を復元）、
`tools/test_layout.py`（後述）。中身は既存の `reviewHtml()`（2026-09-07から未使用のまま
残っていた集計関数）をそのまま再利用しただけで、新しい集計ロジックは足していない。

「読む・書く」ボタンと「時間割に追加」は一覧カードの操作バー（`.cardActs`）に残したまま、
詳細側には出していない（2026-09-07の「同じ操作を画面に2つ並べない」判断は変えていない）。

デスクトップ（1280px）・スマホ（375px）の両方で目視確認ずみ（id=137199「現代の環境を考える」）。

### 2. 何をしていないか

- **本文を1件だけ引用する分岐が、実際の表示では未確認。** ローカルの実データで口コミが
  1件以上ある科目は id=137199 だけで、本文（note）が空だったため、集計数字の表示は
  確認できたが引用文（`「」`で囲む部分）は表示されなかった（条件分岐で正しく非表示になった
  だけで、バグではないはず）。本文入りの口コミがある科目で一度見てほしい。
- **`tools/test_reviews.py` が1件落ちている**（「実データの受講年が全件埋まっている」、
  対象 id=137199）。今回の変更とは無関係 ―― `reviews.load()` が読む実データ側の話で、
  口コミ運用（しゅんやさん担当）の欠損。直さず報告のみ。
- **`tools/test_index_gate.mjs` がこの環境（Windows / Git Bash）で実行できなかった。**
  `worker/index.js` を `import(path.join(...))` で読もうとして
  `ERR_UNSUPPORTED_ESM_URL_SCHEME`（Windowsの絶対パスは `file://` URL が必要）で落ちる。
  同じ立て付けの `test_favorite.mjs` `test_inspector_center.mjs` `test_rail_toggle.mjs`
  は正常に通っているので、このテストだけの環境依存の書き方の問題。今回の変更とは無関係。
- **「図2」の解釈は仮説。** wangさん本人には確認していない（このセッションはDiscord書き込み
  不可）。松下さんとは「9/10の2枚目の添付画像＝口コミモーダルの中身」で合意した上で実装したが、
  wangさんの意図と違っていたら手直しが要る。

### 3. 次の人が最初にやること

    python server.py --port 8000
    python tools/test_layout.py   # 21件通過（後述の assertion 変更込み）

ブラウザで口コミが複数件ある科目を開き、集計数字と引用文が両方出るか確認。

### 4. 今回踏んだ罠

- **`tools/test_layout.py` に「`detailHtml` に `reviewHtml` が残っていたら失敗」という
  assertion が残っていた。** 2026-09-07（口コミを一覧カードの操作バーへ移した時）の仕様の
  まま固定されていて、今回の意図的な仕様変更（集計だけ詳細に戻す）とぶつかって最初 NG になった。
  `panelBtn` / `ttAddBtn` / `reviewBtn`（＝ボタン。cardActs と重複する）はチェックを残し、
  `reviewHtml`（＝集計。対応するボタンが cardActs に無いので重複しない）だけ外した。
  テストを書き換えてバグを黙らせたのではなく、確認済みの仕様変更に追従させただけ。
- **`.dSec+.dSec` の区切り線 CSS が「`.dSec` は1個しかない」という前提で2026-09-07に
  削除されていた。** 今回 `.dSec` が2個（成績評価の内訳／口コミ）に戻ったので、同じルールを
  復元しないと2つのセクションの間に区切り線が出ない。
...
KOAN の `campussquare.do` はログイン不要なので、**必要な137件だけ自分で取り直した**
（`scrape/koan.py` の `detail()`。delay は既定の2秒のまま。137件で約5分、失敗0）。
全7,906件を取り直したわけではない。

### 1. 分かったこと（137件の生HTMLで実測）

**② パーサは表を取りこぼしていない。** `grading()` の結果と、いま焼かれている
`eval_raw` を137件すべてで突き合わせて **不一致0件**。
`%` の無い列を落としている、ということは起きていない。
**表の合計が100%に届かない18件は、シラバスの表そのものが埋まっていない。** これで確定。

**① 表が使えない科目でも、配点は本文の `成績評価に関する補足情報` に書かれていることが多い。**

    137件中 補足情報あり 107件（うち配点の数字入り 63件）
      表が空          59件 → あり 37（数字入り 18）
      案内文だけ      50件 → あり 50（数字入り 40）
      合計<100        18件 → あり 14（数字入り  5）
      案内文まじり    10件 → あり  6（数字入り  0）

    人文地理学演習(003204)  表は空 →「授業で指示する課題類（80％）と
                                     授業での議論への貢献度（20％）で評価します。」
    仏教学講義(001617)     表は60% →「小課題3回（400字程度。各20点）と
                                     レポートでの問題設定能力や論理性（40点）」

**残り30件は補足情報も空。** ここは KOAN へのリンク以外に出せるものが無い。

### 2. 何が動く状態か

    python3 tools/merge_eval_note.py --raw data/raw_hosoku   # 実行済み・コミットに含む
    tools/test_*.py 19本すべて通過
    tools/test_eval_raw.mjs 静的103件 / API 全通過

- `scrape/parse.py` … `eval_note`（`成績評価に関する補足情報`・400字まで）を出力に足した。
  全件のHTMLが揃えば `python3 scrape/parse.py` が同じ値を入れる
- `tools/merge_eval_note.py`（新規）… HTMLを持っている科目だけ `eval_note` を足す。
  **既定の書き込み先は `web/data/courses.built.json`** ―― `data/courses.json` は
  取得した人以外だと共通教育1,112件ぶんしか無く、137件はほとんどが学部の専門科目で
  当たらない（実測：45件中2件しか当たらなかった）。`build.py --rescore` と同じ考え方
- `build.py` … `KEEP` に `eval_note` を追加。**これが無いと次のビルドで消える**
- `app.js` … 表が使いものにならないときだけ本文を出す。表が足りている科目には出さない
  （出すと帯と文章で配点が二重に見えて、どちらが正か分からなくなる）

**％を機械で拾って帯に足すことはしていない。** 「小課題3回（各20点）」のような
書き方が混ざっていて、拾い方を決めた時点でシラバスに無い解釈を足すことになる。
帯はあくまで表の写しで、本文は文章のまま別に出す。

### 3. 次の人が判断すること

- **これはシラバス本文を1フィールドぶん公開に載せる変更**。`build.py` の `KEEP` は
  「原文系は全部落ちる」方針で、`attendance_rule` などは載せていない。
  成績評価の話そのものであること・表が空の137件では他に出せるものが無いことから
  載せる判断にしたが、**気になるなら KEEP から外せば画面から消える**（1行）
- 政岡さんへの依頼は**まだ意味がある**。今回取ったのは137件だけで、
  残り7,769件の `eval_note` は入っていない。全件のHTMLがあれば
  `python3 scrape/parse.py` 一発で揃う

### 4. 罠

- **「HTMLが無いと分からない」は、たいてい「まだ取りに行っていない」だけ。**
  KOAN はログイン不要で、必要な科目だけなら5分で取れる。全件を持っている人を
  待つ前に、**要る分だけ取る**という選択肢を先に潰しておくこと
- `tools/merge_eval_note.py` の書き込み先を `data/courses.json` にすると、
  手元では137件中2件しか当たらない。**当たらないことに気づかず「補足情報が無い」と
  結論しかける**ところだった。全件が入っているのは built のほう
- `build.py` の `KEEP` に足し忘れると、次に誰かが `build.py` を流した瞬間に
  画面から消える。データを足したら KEEP も見ること

---

## 2026-09-07 ｜ 成績評価の内訳を、シラバスの文言そのままに変えた ｜ Claude → 次の人

本人指摘：タイ社会講義IIa の内訳が「レポート 50%」だが、KOAN の原文は
`学習への参加度35% / 発表30% / レポート・論文20% / 小テスト15%`。

はじめ「発表」を区分として足す方向で作ったが、**本人判断で方針を変えた**（同日）：

> 評価成分は全部シラバスに基づく。シラバスに test が無ければ書かない。数字もシラバスと同じ。
> **こちらで統合したり名前を付け替えたりしない。**

なので `evalCompHtml()` は `eval_raw`（KOAN の成績評価テーブルの行そのもの）を出す。
文言・数字・並び順すべてシラバスのまま。**採点側は一切触っていない。**

### 1. 何が動く状態か

    python3 server.py --port 8146
    tools/test_*.py 19本すべて通過
    tools/test_eval_raw.mjs：静的98件 / API86件 すべて通過（新規・シラバスとの一致を見る）
...
- `app.js` … 「ここには書いていない」とだけ書かれている行を**灰色**にした（15種類・60箇所、
  うち49科目は帯がこれ1本だけ）。`補足情報を参照 100%` を成分と同じ色で塗ると
  「補足情報を参照という科目が成績の100%」に見える。**文言と数字はシラバスのまま**で、
  変えたのは色と、その下に出す注記だけ。本人判断・2026-09-07。
  🚨 `その他（レポート、課題提出、小テスト、発表、学習への参加度）` のような
  **中身を並べている行は成分**なので灰色にしない。似た21種類（`その他の課題`
  `出席カードへのふり返り記入` など）を全1,134種類に当てて目視で分けた。
  判定を増やすときは同じことをやること
- `score.py` / `app.js` … 一覧のカードの「判定不可」の理由文を**言い分ける**ようにした。
  内訳を直写しにしたことで、**詳細に内訳が4項目・合計100%で出ているのに、一覧のカードで
  「シラバスに成績評価の内訳が載っていない」と言う**状態になっていた（507科目）。
  実際に載っていない59科目とは別物なので、`eval_unclassified` の有無で分ける。
  **`score.py` で変えたのは文字列だけで、スコアは1点も動いていない**
  （`_unjudged_reason` は built に焼かれず、その場で組み立てられる。焼き直し不要）
- `web/about.html` … 「スライダーの数字はシラバスに書かれている割合そのもの」という
  説明が**この変更で嘘になった**ので直した。しぼり込みは今までどおり4区分に寄せた値を
  使うので、詳細の内訳（シラバス直写し）とは1対1で対応しない。その食い違いを先に書く
- `tools/test_eval_raw.mjs`（新規）… **凡例の文字列がシラバスの表と一字一句・一数字
  ...
  🚨 **API モードと静的モードの両方で流すこと。**

      python3 server.py --port 8149 &          → node tools/test_eval_raw.mjs http://127.0.0.1:8149
      (cd web && python3 -m http.server 8150) & → node tools/test_eval_raw.mjs http://127.0.0.1:8150

  **静的モードでしか踏めない科目がある。** server.py が配る `data/courses.json` は
  取得した人以外の手元では共通教育1,112件ぶんしか無く、そこに未分類の科目は**0件**。
  今回の変更の動機のど真ん中が API モードでは1件も踏めない。
  静的モードは `courses.built.json` の全7,906件を読むので踏める。
  科目は `?c=<id>` の深リンクで開く（タイトル検索は全角カッコなどの表記ゆれで外す）。
  踏んでいる2種類（実測）:

      4区分に1つも入らない   93科目 … eval_ratio が null。以前は内訳の欄そのものが出なかった
      4区分に一部しか入らない 414科目 … 落ちたぶんが灰色の「不明」に名前ごと消えていた

  旧挙動（4区分に振り分けて出す）を注入すると **94件中57件が NG** になることを確認ずみ
- `tokens.css` … `--comp-attendance` 等を **`--comp-1`〜`--comp-5` ＋ `--comp-gap` に改名**。
  ...
  KOAN の成績評価テーブルは**最大5列**なので5色で足りる（全7,847件で実測）

### 2. これで同時に消えた3つのズレ（全7,906件で実測）

    ① 発表がレポートとして出ていた
       「発表」の出現 延べ2,284回（単独の項目名では4番目に多い）。
       含む科目 2,579件（32.6%）・配点の中央値40%。
       **うち1,363件は report の中身が発表だけ**＝レポートは1本も無いのに
       「レポート」と表示していた（タイ社会講義Ia は 学習への参加度20% ＋ 発表80%）
    ② 期末レポート・期末課題が「期末テスト」として出ていた
       METHOD_RULES の試験ルールに裸の「期末」「中間」が入っているため。延べ172箇所。
       環境評価論は `期末レポート 100%` の1項目だけなのに「試験100%」だった
    ③ 振り分けられなかった項目が灰色の「不明」に消えていた
       723箇所 / 507科目。**中央値で35%の配点が名前ごと消えていた**（50%以上
       消えていた科目が195件）。予防看護学実習は「実習への取り組み＋実習の記録」で
       100%全部が消えていた

シラバスの行をそのまま出すので、3つとも起きようがなくなった。

### 3. 次の人が最初に知るべきこと ―― ここが一番大事

**画面のこの部分だけがシラバス直写しになった。採点・つまみ・条件チップは
今までどおり `eval_ratio`（4区分）を見ている。** 上の②③は**採点側にはまだ残っている**：

- `期末レポート 100%` の科目は、画面には「期末レポート 100%」と正しく出るが、
  **スコアの上では試験100%として扱われている**（`_exam_load` が効く）。
  条件チップ「レポートのみ」にも出てこない
- 未分類が残る科目は `evalKnown()` が false なので、つまみを1本でも動かすと結果から外れる。
  画面には配点が全部出ているのに絞り込みからは消える、という食い違いが起きうる

裸の「期末」「中間」を外す修正は**本人判断で見送っている**（「裸词不需要移除」）。
やるときは §5 の罠を先に読むこと。

### 4. 何をしていないか

- **`score.py` `scrape/parse.py` は 1行も変えていない。** バケツも METHOD_RULES も main のまま
- **複合項目「その他（A、B、C）」型は、シラバスがそう書いているのでそのまま出す。**
  最大が `その他（レポート、課題提出、小テスト、発表、学習への参加度）` 120件。
  表示としてはこれで正しい（シラバスの文言そのもの）が、**採点では「小テスト」が
  先に当たるので5類ぜんぶの配点が小テストに入っている**
- 項目名が長いものはそのまま折り返す。最長100字（`研究の実施（１００％）。研究成果を…`）。
  1,134種類あるので省略規則を作るとそれ自体が「言い換え」になる
- `tools/test_haiten_ui.mjs` の `既定で全件出ていない: 1112件` は**環境要因**。
  server.py が配る `data/courses.json` は手元では共通教育1,112件ぶんだけで、
  全所属7,906件は取得した人（政岡さん）の手元にしか無い。**main でも同じように落ちる**（実測）

### 5. 今回踏んだ罠

- **「システムの都合の分類」を画面に出していないか、まず疑う。** 今回のズレは3つとも
  「採点のために作った4区分を、そのまま学生に見せていた」ことが原因だった。
  振り分けは採点の都合であって、学生が見るべき事実ではない
- **ルールから語を外すときは、外した後どこへ落ちるかを全件シミュレーションしてから外す。**
  試験ルールから裸の「期末」「中間」を外す案を試した時点で、`期末ディベート`
  `達成度チェック` `期末リポート` の8箇所が未分類に落ちることが分かった。
  未分類は画面では灰色になるだけで**エラーにならない**ので、テストでは気づけない
- **色トークンに意味の名前を付けると、意味が変わったとき嘘になる。**
  `--comp-report` は導入時（2026-09-05）は正しかったが、内訳の出し方が変わった瞬間に
  「レポートの色で発表を塗る」になった。並び順の名前なら腐らない
- **`data/courses.json` は gitignore なので、本体では常に「焼かれた時点のバケツ」のまま。**
  `eval_raw` は生のまま入っているので今回の表示には影響しないが、
  `eval_ratio` を触る変更をするときは `tools/rebucket.py` を流してから画面を見ること

---

## 2026-09-07 ｜ 2列のとき ※ の行が1行ぶん切れていた ｜ Claude → 次の人

昨日 ※ の行を 13px に上げたら、**PC の2列レイアウトで下の行が消えていた**（本人指摘）。
原因は文字サイズではなく**置き場所**。`.bandNote` は `.reason` の中にいて、
`.reason` は 768px 以上で `-webkit-line-clamp:2` で切られる。13px にして折り返しが
2行になったぶん、はみ出した行がクランプに食われていた。

**実測（1280px・「統計学」で絞り込み・24枚）**

    直す前   24枚中 19枚で .reason が 表示46px / 実寸66px ―― 1行（20px）欠け
    直した後 24枚中  0枚。※ の行は2行ぶん 42px を全部出す

### 1. 何が動く状態か

    python3 server.py --port 8811
    tools/test_*.py 19本すべて通過（haiten_filter 49/49・layout・tokens ほか）
    tools/test_*.mjs ＋ smoke.mjs：下の2本を除いて全通過

- `app.js` … ※ の行を `.reason` の**外**へ出した（`.head` の grid に直接ぶら下げる）
- `app.css` … `.bandNote` に `grid-column:1/3`。詳細パネルでも同じクラスを使うが、
  あちらは grid ではないので無視される

### 2. 何をしていないか

- **版番号には載せていない**（本人判断・2026-09-07）
- `.reason` のクランプ（2行）はそのまま。理由文はこれまでどおり2行で頭打ち
- 2列時のカード下限 `--cardH:224px` は触っていない。`min-height` なので、
  ※ が2行になる科目は行ごと伸びる（切らない、という元の設計どおり）

### 3. 次の人が最初にやること

    git worktree list        # .worktrees/notefull が残っていたら remove

### 4. 今回踏んだ罠 ―― ここが一番大事

- **カード内の文字を大きくするときは、その要素がクランプの中にいないか先に見る。**
  `.reason` は「band チップ＋理由文」を2行で頭打ちにする箱で、※ の行はその同居人だった。
  字号だけ見ていると「PC で見えない」が再現できない（**スマホ幅では起きない**）
- **検証は「見えているか」ではなく「祖先の overflow に食われていないか」で測る。**
  最初 `.bandNote` の bottom を `.reason` の bottom と比べていたが、外に出した後は
  その比較自体が無意味になる。祖先を遡って `overflow!==visible` または
  `-webkit-line-clamp` を持つ箱からはみ出していないかを見るのが正しい
- `tools/test_haiten_ui.mjs`（「既定で全件出ていない: 1112件」）と `tools/test_onboard.mjs`
  （onboard ダイアログが click を遮る）は**この変更の前後で同じように落ちる**。
  前者は 7,906件の全データが要る検査で、worktree に置いた 1,112件では通らない
...
カードの `情報不足` の横に出ていた
    ...
    判定に必要な情報が足りていません。口コミが3件そろうと出ます。

を**消した**。同じことを band の下の ※ の行（投稿への誘い）が言うので、
1枚のカードに口コミの話が二段で並んでいた。※ の行に一本化し、そのぶん
文字を 11.5px → 13px（`--fs-xs` → `--fs-sm`）に上げた。

### 1. 何が動く状態か

    python3 server.py --port 8801
    node tools/smoke.mjs http://127.0.0.1:8801            # ✓ コンソールエラーなし
    node tools/test_index_gate.mjs http://127.0.0.1:8801  # OK 37件
    python3 tools/test_haiten_filter.py                   # 49/49
    python3 tools/test_layout.py / test_tokens.py / test_shell_inject.py
    python3 tools/test_recommend_order.py / test_scoring_gate.py

- `score.py::_unjudged_reason()` … 「口コミ待ち」の枝だけ**空文字**を返す。
  待っても出ない2つ（内訳が無い／読み取れた%が足りない）は今までどおり文を返す
- `app.js::matchLocal()` … 静的モードも同じく空文字（片方だけ直さない）
- `app.js::bandNoteText(c)` … ※ の行を出す条件を広げた。`needs_review` だけでなく
  **「総合値がまだ出ず、その穴が口コミで埋まる科目」**にも出す
- `app.css` … `.bandNote` を `--fs-sm`（13px）へ

### 2. 何をしていないか

- **版番号（右下の「バージョン＆最新機能」）には載せていない**（本人判断・2026-09-06）
- `判定不可`（シラバスに内訳が無い科目）の文言は触っていない。※ の行が出ない科目なので、
  ここを消すと本当に何も言わないカードになる
- `/kuchikomi` への直リンクは相変わらず張っていない（カード見出しが展開ボタンでタップが競合）

### 3. 次の人が最初にやること

    git worktree list        # .worktrees/bandnote が残っていたら remove

### 4. 今回踏んだ罠 ―― ここが一番大事

- **「文を消すだけ」で終わらせると 451件が説明ゼロになる。** その文が出ていた 552件のうち
  **451件は ※ の行が出ない**（成績評価に試験が無い科目は `needs_review` が false）。
  だから ※ の行を出す条件のほうを広げた。**消す前に「代わりに何が出るか」を数えること**
- 条件を広げた結果、**31件は文も ※ も出ない**（口コミが1〜2件あって門を越えていない、
  かつ試験が無い科目）。ただしこの31件は全部 `scored:false` なので
  **注意帯「⚠ 口コミ N件 ― まだ数字には入っていません」が出る**ことを確認済み。
  カードが無言になる組み合わせは無い
- `tools/test_haiten_filter.py` ⑥ が「口コミで埋まる科目の reason に『口コミ』が入っている」
  ことを検査していた。文言を消すならこのテストも一緒に直す（期待を「空文字」に反転させた）
...
何も選んでいなくても `grid-template-columns` が確保したままだったのが原因。
**触ったのは PC（1160px 以上）の並びと、詳細の閉じ方だけ。** API・データ・
スコアの算出・スマホの見た目は触っていない。

### 1. 何が動く状態か

    python3 server.py --port 8798                              # 別窓で
    node tools/test_inspector_center.mjs http://127.0.0.1:8798  # ✓ 19件（新規）
    node tools/test_rail_toggle.mjs      http://127.0.0.1:8798  # ✓
    node tools/test_favorite.mjs         http://127.0.0.1:8798  # ✓ 38件
    node tools/smoke.mjs                 http://127.0.0.1:8798  # ✓ コンソールエラーなし
    python3 tools/test_layout.py && python3 tools/test_tokens.py

入ったのは3つ:

- **空のときは右カラムを畳み、絞り込み＋一覧を中央に置く**。一覧は 860px
  （本人決定）。1160〜1176px では入りきらないので `min(860px, calc(100% - 284px))`
  で容器いっぱいまで縮める
- **詳細を ✕ / Esc で閉じられる**（`closeDetail()`）。これが無いと、いちど科目を
  押した人は再読込するまで中央寄せの画面に戻れなかった
- **空のとき（2列）のカードを等高に**。実測で 132〜202px ばらついていた

### 2. 何をしていないか

- **「畳んだときだけ2列」という約束は退役した。** 860px は2列のしきい値 770px を
  越えるので、空なら畳む前から2列。`--cardMin` のコメントと
  `tools/test_rail_toggle.mjs` を併せて直してある（あの test は科目を1件選んでから
  測るようにした ―― 畳んで中カラムが広がるのは選択中の話になったため）
- **科目を選んだ瞬間、一覧は 860px 1列 → 640px 2列→1列に組み直る。** 本人承知のうえ
- **口コミの注意帯（.rvAlert）が入る科目だけは高い**（249/291px）。帯を切るより
  行ごと伸ばす選択。同じ行の相方は `align-items:stretch` で追従する
- `:has()` を解さないブラウザでは、これまでどおり右を空けた3カラムに落ちる
- スマホ幅（1160px 未満）は一行も変えていない

### 3. 次の人が最初にやること

    git worktree list            # .worktrees/inscenter が残っていたら回収してよい

### 4. 今回踏んだ罠

- **`.meta` に `white-space:nowrap` を足したら、楽単スコアが星の下に潜った。**
  grid の 1fr 列は自動最小幅が min-content（＝nowrap した行の全長）まで膨らむので、
  右の 58px 列を押し出す。`min-width:0` で切る。`tools/test_favorite.mjs` が
  この重なりを見張っていたおかげで気付けた
- **カードの高さを静的スナップショット（`web/data/`・7,906件）だけで測ると足りない。**
  そこでは理由が全部1行で 202px に収まるが、`server.py` の実データでは2行に折り返す
  科目があり 224px 要る。**高さを決める測定は `server.py` 側でもやること**
- 直したはずの値が効かないときは、同じセレクタが後ろの `@media` に残っていないか見る。
  今回 1440px 側に古い 736px の指定が残っていて、そちらが勝っていた
