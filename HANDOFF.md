...
## 2026-08-26 ｜ 旧ドメインを独自ドメインへ 301 で寄せた ｜ wang → 次の人

ブランチ `feat/wang-old-domain-301`（作業ツリー `../rakutan-old301`）。
直前の追記「旧ドメインの noindex が効いていなかった」の根治。

### 1. 何が動く状態か

**旧 `rakutan-db.wjy20050815.workers.dev` を開くと、独自ドメインの同じページへ飛ぶ。**
ただし **LINE の Webhook と API は転送しない**（301 は POST を GET に変えるため）。

```bash
node tools/test_index_gate.mjs        # 29件
npx wrangler dev --port 8788 --local  # 本物の Workers ランタイムで確かめる
curl -sI -H "Host: rakutan-db.wjy20050815.workers.dev" http://localhost:8788/          # → 301 独自ドメイン
curl -s  -H "Host: rakutan-db.wjy20050815.workers.dev" http://localhost:8788/line/health # → ok（転送されない）
```

| 旧ドメインへのリクエスト | 結果 |
|---|---|
| `/`・`/about` などページ、`/l/<slug>`、`robots.txt` | **301** 独自ドメインの同じパス＋クエリへ |
| `/line/*`（Webhook・health） | 転送しない。noindex だけ付ける |
| `/api/*`（意見箱・お気に入り） | 転送しない。noindex だけ付ける |
| `localhost`（`wrangler dev`・`http.server`） | 転送しない（開発が詰むため） |

**効かせるのに `wrangler.toml` の `[assets]` に `run_worker_first = ["/*", "!/assets/*", "!/data/*"]` が要った。**
静的アセットは既定で Worker より先に配られるので、これが無いと転送コードが走らない。
`assets/` と `data/`（12MB の JSON）は除外してあるので、Worker を通るのはページだけ。

### 2. 何をしていないか

- **LINE Developers に登録されている Webhook URL を確認していない。** 手元に
  `LINE_CHANNEL_ACCESS_TOKEN` が無いため。**確認しなくても安全な形**にしてある
  （どちらのドメインに POST されても `/line/*` は転送せずそのまま処理する）。
  もし登録先が旧ドメインなら、コンソールで独自ドメインに変えたうえで
  `workers_dev = false` にすると旧ドメインごと消せる ―― そこまでやれば一番きれい
- **ページが Worker を1回通るようになった。** 無料枠（10万リクエスト/日）に対しては
  桁が違うので問題ないが、`assets/`・`data/` を除外から外すと一気に増える。触らないこと
- Search Console 未登録・科目個別URLの sitemap 未収録は据え置き（別項）
...
curl -sI https://rakutan-db.wjy20050815.workers.dev/ | grep -i location   # → 独自ドメイン
curl -s  https://rakutan-db.wjy20050815.workers.dev/line/health           # → ok
```

### 4. 踏んだ罠

- **`wrangler deploy --dry-run` は設定が通ることしか教えてくれない。** `run_worker_first` が
  実際に効くかは `wrangler dev` に `Host:` ヘッダを付けて叩くまで分からなかった。
  設定の受理と挙動は別物
- **`_redirects` では解けない。** Cloudflare の静的アセットの `_redirects` は
  パスにしかマッチせず、ホスト名で分岐できない。両ドメイン同じファイルを配っているので、
  書くと独自ドメインまで自分自身へ転送し始める
- **`smoke.mjs` をローカルで動かすとコンソールエラーが2件出るようになった。**
  Web Analytics のビーコン（#68）が `localhost:8788` からの POST を CORS で弾かれるため。
  **本番では出ない**（`node tools/smoke.mjs https://rakuhan.nocode-sol.co.jp` で確認ずみ）。
  ローカルのこの2件は無視してよい
