...
### 追記（同日）── ヘッダを縦組み・濃色帯にした

指示により、**マークを上・ロゴタイプを下**に積み、**ラ以外を白**にしました。
白い文字が乗るので地は濃色（ブランド資料の SUMI `#1A1A1A`）です。

⚠️ **ここはブランド資料と違います。** 資料の「濃色の上・写真の上」の適用形は
**マークも白抜き**（`markRaku(64, "#FFFFFF")`）で、
「マークだけオレンジ、文字は白」という組み合わせは資料に載っていません。
縦組みのロックアップも資料には無く、横組み（`lockup()`）だけです。
**意図的な判断であれば問題ありませんが、資料と食い違っていることは記録しておきます。**
