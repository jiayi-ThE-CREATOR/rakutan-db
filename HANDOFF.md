...
現在: 24科目・**一言14件**が公開されます。

### 2. 何をしていないか ―― ここが一番大事

- 🚨 **本文を1件だけ止めています。** `data/reviews.json` の
  「誰かに出席カードの記入頼めば行かなくていいです」（135327）に
  `"publish": false` を立てました。**代返の指南**で、学生団体が運営する公開サイトに
  載せる内容としては、大学から止められうる種類のものだと判断したためです。
  `CLAUDE.md` の「過去問そのものは載せない」と同じ族のリスクと考えています。
  **異論があれば `publish` を消すだけで出ます。判断はしてください。**
  なお**止めているのは本文だけで、行は残ります** ―― その口コミの出席や
  テスト難易度は正しい情報なので、件数にも採点にも効き続けます（135327 は
  `n=2` のまま、一言だけ1件）。
- **残り19件は目を通しました。教員名への言及は0件です**（README 5章の禁止事項2）。
...
本文を止めたいときは `data/reviews.json` の該当行に `"publish": false` を足して
`python3 build.py` を流し直すだけです。

### 4. 踏んだ罠

- **`data/reviews.json` は gitignore です。** つまり `publish: false` という
  **審査の結果は git に乗りません。** ファイルを持っている人（いまは wang と政岡さん）が
  build.py を流したときだけ効きます。**別の人が流すと止めたはずの本文が出ます。**
  口コミが増えたら、審査結果の持ち方を決め直す必要があります。
