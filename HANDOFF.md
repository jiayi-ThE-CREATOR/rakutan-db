...
## 2026-08-20（8） ｜ サイトの口コミ投稿を、仮フォームと同じ設問に揃えた ｜ wang

### 1. 何が動く状態か

```bash
python3 server.py     # → http://localhost:8000 で「口コミを書く」を押す
```

サイトの投稿シートが **9問**になりました。**入口A（netlify の仮フォーム）と同じ内容 ＋
学部・履修時の学年**です。保存される1行も `data/reviews.json` の既存36件と同じ形なので、
`reviews.py` がそのまま読みます。

```
1 どの科目？            6 授業外の課題は？（重い/ふつう/軽い/なかった）
2 あなたの学部は？        7 テストはあった？ → 持ち込み（可/不可）・難易度（1〜10）
3 取ったときの学年は？     8 レポートはあった？ → 語数
4 出席は取られた？        9 一言（任意）
5 授業中の課題は？
```

**入口A は「公開前に第一手のデータを集める」ための仮の入口**で、公開後の投稿口はサイト側
（入口B）です。A を将来また回す可能性を考えて、`tools/ingest_reviews.py` も
`faculty` / `student_year` のキーを出すようにしました（列が無ければ `null`）。

**一言の欄に注意書きを2つ出しています。**

> **先生個人のことは書かないでください。**名前・人柄・評判など。
> **評価の対象は科目であって人ではありません。**
> 書いた一言は**そのままサイトに公開されます**（誰が書いたかは出ません）。

前者は README 5章の禁止事項2（「投稿ガイドラインに明記し、投稿前の注意書きに出す」）が
求めているものです。後者は、一言をサイトに公開する方針に変えた以上（別 PR）、
**書く人がそれを知らないまま書くことになるのを避けるため**です。
同じ主旨をシート冒頭の説明にも1行入れています。

**書いた内容のチェックはしていません。** 注意書きだけで、NGワードも通報導線もありません。
**これで A 由来の行と B 由来の行が同じ形になります。**

### 2. 何をしていないか

- ⚠️ **`web/index.html` を触りました（松下さんの担当ファイル）。** 投稿シートの
  設問・その JS・シート用の CSS 3行です。一覧・カード・チップには触れていません。
- ⚠️ **公開サイト（Cloudflare＝静的）では、この投稿口はまだ出ません。**
  静的ホスティングには受け皿が無いので `CAN_POST = false` のままで、FAB ごと
  隠れています（この判断自体は既存のもの）。**入口Bを本当に本番の投稿口にするには
  D1 なり Functions なりの保存先が要ります。そこは手を付けていません。**
  いま動かして確認できるのは `python3 server.py` のときだけです。
- **学部ごとの集計・表示はしていません。** 保存するだけです。`reviews.py` の集計は
  変えていないので、`courses.built.json` にも学部・学年は出ません（＝公開されません）。
- 既存36件には学部・学年がありません。**必須なのは入力フォーム側だけで、データとしては
  欠けていてよい**という前提です。
- 学部の選択肢は阪大11学部＋「その他」。**大学院・他大学は想定していません。**

### 3. 次の人が最初に打つコマンド

```bash
git fetch && git checkout feat/wang-review-form
python3 server.py          # → localhost:8000 → 「口コミを書く」
```

`data/reviews.json` に本当に書き込まれるので、**試したら最後の1行を消してください**
（採点に混ざります）。設問を増やすときは **`web/index.html` のシート・`server.py` の
`do_POST`・`tools/ingest_reviews.py` の `normalize()` の3箇所**を必ず一緒に直すこと。

### 4. 踏んだ罠

- **「なかった」を 0 にしてはいけません。** 授業中／授業外の課題は
  重い2・ふつう1・軽い0・**なかった=null** で、`ingest_reviews.py` の `LEVEL` も
  そうなっています。元の JS は `+b.dataset.v` で数値化していたので、そのままだと
  「なかった」が `+"" === 0`（＝軽い）に化けます。**回答は文字列のまま持ち、
  数値化は送信直前だけ**にしました。`""`（＝なかった）と `undefined`（＝未回答）は別物です。
- **テストの難易度はスライダーの既定値のまま送らせてはいけません。** 触って初めて
  回答扱いにしています（未タッチだと送信ボタンが有効になりません）。既定の5が
  そのまま入ると、口コミが一番効く値に「ふつう」が量産されます。
- **`reviews.json` を書くときの `indent` は `ingest_reviews.py` と揃える**（1）。
  以前 `do_POST` は 2 で書いていたので、1件投稿しただけでファイル全体が
  書き換わり、差分が読めなくなるところでした。
- **消えた設問について。** 以前の `workload` / `grading` は `reviews.py` が
  一度も読んでおらず、採点には1ミリも効いていませんでした。逆に一番効く
  「テストの難易度」「レポートの語数」はサイト側では聞いていませんでした。
  今回それが揃います。
...
bad=[c for c in d["courses"] if c.get("eval_ratio")
     and sum(c["eval_ratio"].values())<99.9 and c["rakutan"]["overall"] is not None]
left=[c for c in d["courses"] if c.get("eval_unclassified")]
print("欠けたまま点数が出ている:", len(bad), "件（0であること）")
