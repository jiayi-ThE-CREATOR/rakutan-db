...
## 2026-08-22（13） ｜ 科目詳細からKOAN公式シラバスへ直リンク ｜ 松下

政岡さんの提案（`政岡_シラバス直リンク_20260820.md`）を実装。

### 1. 何が動く状態か

科目カードを開く（詳細を展開する）と、口コミボタンの上に
「この科目のKOAN公式シラバスを見る ↗」というリンクが出ます。押すと別タブで
その科目・そのクラスのKOAN公式シラバスが開きます。

```bash
python3 server.py     # → http://localhost:8000 を開き、任意のカードを開いてリンクを押す
```

変更したのは `web/index.html` の3箇所だけ：
- `detailHtml()` にリンクを追加、`koanUrl(id)` で `j_cd=<時間割コード>` を組み立てる関数を新設
- `.koanLink` のCSSを新設（口コミボタンとは別系統の弱い下線リンク。主導線と競合しない見た目）
- フッタの「KOANで確認してください」の文言に「各カードの『KOAN公式シラバスを見る』から確認できます」を追記

`data/` `scrape/` `build.py` には触っていません。既存の `courses.built.json` の `id`
（全1,112件が6桁の時間割コード、重複なし）をそのままURLに使うだけで済んだため、
データの追加取得も再ビルドも不要でした。

### 2. 何をしていないか

- **時間割コードは画面に表示していません。** 同名の科目が複数クラスある場合
  （例:「【人文】欧米の文化と社会を知るB」が `135073` と `137065` の2クラス存在）
  でもリンクはクラスごとに正しく分岐しますが、学生がコードそのものを見て
  区別する手段は無いままです。表示するかどうかは未決定・要相談のまま持ち越し
- **リンクは詳細を開かないと出ません。** カードの見出し行（閉じた状態）には出していません
- `nendo=2026` は決め打ちです。来年度以降このサービスを使い回すなら書き換えが要ります
- LINE側（きむらさん担当）にはこのURL形式を**共有していません**。実装もしていません

### 3. 次の人が最初にやること

このタスクとしてはやることはありません。政岡さんへ「実装済み・確認済み」と返信を。

### 4. 今回踏んだ罠

- **`/api/courses?year=1` が0件を返す既知の不具合を発見しました（今回の変更とは無関係）。**
  `year=all` なら1,112件正常に返るので、学年フィルタ側（`server.py`、wang担当）の
  バグの可能性が高いです。**このタスクの範囲外なので修正していません。** 公開前に
  wangさんへの確認をお願いします
- ブラウザの検証中、`computer`（クリック操作）ツールがプレビューペインの表示不良で
  タイムアウトすることがありました。`javascript_tool` で `element.click()` を
  直接呼ぶ・`read_network_requests` でAPI応答を直接見る、で代替できました
- URLに `?year=all` を付けてページ遷移しても、アプリ側は起動時に `location.search`
  を読んでいないため学年フィルタには反映されません（フィルタ状態はチップの
  クリックでしか変わらない）。検証時に気づかず一度ハマりました
