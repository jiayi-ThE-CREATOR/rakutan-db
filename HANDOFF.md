...
```bash
open tools/tile-demo.html     # サーバ不要。実データ52件を埋め込み済み
```

一覧を「小さいタイル」にしたときの見え方を触って確かめられる。スライダーでタイル幅 110〜280px、
「いまのカード」との切り替え、電光掲示板（溢れた科目名を流す）の on/off。
**タイルを押すと、そのタイルだけ横いっぱいに開いて「重さの根拠」（4軸のバー＋根拠文＋信頼度）が出る**
（Finder のアイコン表示と同じ。もう一度押すか Esc で閉じる）。52件すべてを開いた状態で描画して、
エラーなし・根拠の行208（52件×4軸）・「未取得」114行・総合値なし6件 を確認ずみ。
右上に **列数・実寸・1画面あたりの件数・根拠バーの本数** が出るので、密度をカードと数字で比べられる。

**`web/` は1バイトも触っていない。** `tools/` 配下なので Cloudflare Pages にも出ない。

**実データを測って分かったこと（7,906件）**

| 調べたこと | 結果 |
|---|---|
| 科目名の幅（全角換算） | 中央値 **9文字**、p90 18、p99 27、最長 61 |
| 3列（1行7文字）で2行に収まる | 69〜76%。**3行なら96%** |
| 2行に収まらない科目 | **294件＝3.7%**。うち87%が「◯◯（…）」型（学問への扉79・総合英語48・スマスポ37） |
| 【総合】を外し、括弧の前を主見出しにすると | 2行のままで **69.2%→93.6%**（139px）／**76.3%→96.1%**（149px）。該当2,519件＝31.9% |
| 一覧が入る `.results` の実幅 | スマホ358 / md 472-664 / lg 440-680 / xl 760px |

**列数の結論**：既定は3列。スマホは2列が上限（3列＝111pxで科目名が5文字/行）。
**4列が成立するのは1440px以上だけ**（lgで4列＝101px、全タイルが「【総合】文」で終わる）。
`auto-fill` の自動列数は使わない ―― xl で5列に増えて逆に読めなくなる。

### 2. 何をしていないか

- **一番効く事実：手元のデータでは4軸のうち平均1.48本しか値が無い。**
  出席 100% / 試験 46.4% / レポート・課題 **1.9%** / 規模・形態 **0%**。
  **バー1本だけの科目が52.5%、2本が46.7%。** つまり「小さくして一眼で情報量を増やす」効果は
  **口コミが増えるまで出ない**（レポートの本数・字数は口コミでしか埋まらない）。
  デモの52件は口コミ付きを14件わざと混ぜてあるので、実際より賑やかに見える。
- **手元のツリーが公開版より古い。** スクショの「楽単スコア」「小テストの少なさ」の軸が
  手元の `app.js` / `courses.built.json` に無い（手元は `.fit`＝「相性」、軸は試験/レポート/出席拘束/規模）。
  **上の集計は公開版で取り直す必要がある。**
- ブラウザで実際に開いて見ていない（この環境に node が無く、`tools/shots.mjs` を回せなかった）。
  電光掲示板は `100cqw`（コンテナクエリ）を使っているので、**Safari で要確認**。
- お気に入り☆・ページ送り・絞り込み・KOANリンク・口コミ本文はデモに入れていない。見え方だけ。
- `web/index.html` `app.js` `app.css` は未変更。ビュー切り替えの実装計画はまだ無い。

### 3. 次の人が最初に打つコマンド

```bash
open tools/tile-demo.html
# スライダーを110pxまで下げて、どこで数値が読めなくなるかを見る
# 「電光掲示板」を入れて、学問への扉の長いやつにカーソルを乗せる
```

決めてほしいのは3つ：
1. 科目名は**2行固定＋括弧の前を主見出し**（推し）か、3行固定か
2. 電光掲示板は「溢れたタイルに触れている間だけ」でよいか
3. `TOP_PICKS = 5` が3列・4列で歯抜けになる。6件にするか、この枠だけカードで残すか

### 4. 踏んだ罠

- **`-webkit-line-clamp` が効いている要素は、幅も高さも測れない。**
  `scrollWidth` は枠幅と同じ値を返すので、流す量が常に0になり電光掲示板が動かない。
  → 流す量は CSS の `translateX(calc(-100% + 100cqw))` で出し、
  はみ出し判定だけ JS で `webkitLineClamp="unset"` に戻してから高さを測る。
- **`prefers-reduced-motion` と電光掲示板は両立しない。** ROADMAP 8.4 は全モーションを1msに落とすと決めている。
  止めた瞬間に情報が消えるので、**電光掲示板は省略（…）の代わりにはならず、上乗せにしかならない。**
  静止時の「…」は必ず要る。
- **ROADMAP 8.4「オレンジの小さい文字は使わない」**があるので、LED風（黒地にオレンジのドット文字）は不可。
- **タイルの高さを px で決め打ちすると、科目名が上下に切れる。**
  `.tile` は flex コンテナで、子の既定は `flex-shrink:1`。中身が `height` に入り切らないと
  **科目名の行だけが潰れて字が切れる**（軸ラベルが長くなる大サイズで発生）。
  → 子に `flex:none` を当てたうえで、描画のたびに「いちばん背の高いタイルの自然な高さ」を
  JS で測って全枚数に配る（`fitHeight()`）。決め打ちの数値は捨てた。
- スクショだけ見て実装を始めると、**手元のデータに無い軸を実装しようとする**。先に版を合わせること。

---

## 2026-09-03 ｜ 口コミの「テスト難易度」がシートに入っていなかった（**適用済み・実データで検証ずみ**） ｜ Claude → しゅんやさん

### 1. 何が動く状態か

**シートに「テストの難易度」列（P列）が入り、本番で3件流して検証ずみ。**

| テスト投稿 | テスト列 | 難易度列 | |
|---|---|---|---|
| 【社会】日本国憲法 | `不可` | **`10`** | スライダーの値が届く（初期値5の素通しではない） |
| 【社会】法学の考え方 | 空欄 | **空欄** | テスト無し → **`0` ではなく空欄**。ここが要点 |
| 【社会】行動学の話題 | `可` | `5` | 右隣のレポート有無・字数もずれていない |

難易度1の追加テストは不要。スライダーの最小値が1なので `0` が入る経路はテスト無しの時だけで、
それが空欄になることは上で確定している。

**直す前の状態**: `/kuchikomi` の「テストの難易度（1〜10）」が**シートに1件も入っていなかった**。
経路を端から追った結果、**GAS の書き込み配列に `examDifficulty` を読む行が最初から無い**のが原因。

```
入力欄 kuchikomi.html:308  → ある
送信   kuchikomi.js:782    → 送っている（examDifficulty）
中継   worker/index.js:724 → 素通し
GAS    doPost の配列       → ★ここで捨てている（18要素・難易度が無い）
シート                     → 列が無い
```

**GAS のコードの写しを初めて repo に置いた**（これまでどこにも無かった）:

```bash
diff tools/gas/kuchikomi_before_20260903.gs tools/gas/kuchikomi_after_20260903.gs
#   → 差分は2か所だけ（見出し配列と書き込み配列に1つずつ足すだけ）
```

手順・戻し方の全文: `docs/plans/2026-09-03-kuchikomi-difficulty-column-plan.md`（§11 が実行手順）

**効く大きさ**: `exam_hard10` は `reviews.py:71` の重みが 4（他は 2）。
`HANDOFF.md:3854`「load に最大44点効く」＝**一番重い軸**が丸ごと欠けていた。
同じ形の事故は2回目（1回目は `HANDOFF.md:3954`、フォームのキーと集計のキーのずれ）。

### 2. 何をしていないか

- ★**テスト投稿3件（`135115` / `135151` / `135149`、いずれも 2026-09-03）をシートから消す。**
  消し忘れると口コミ3件として集計に入る（`reviews.MIN_FOR_SCORING` は3件）
- ★**作業中（列を挿してからデプロイするまでの数分）に届いた本物の投稿が無いか未確認。**
  在ればレポート有無から右が1列ずれて入っている。タイムスタンプで探して手で1つ右へずらす
- ★**過去に入った口コミの難易度は復元できない。** 送られてはいたが GAS が捨てているので
  どこにも残っていない（Discord 通知にも載せていない）。**列を足した日以降だけが埋まる**
- **`tools/ingest_reviews.py` はこのシートの書き出しをそのまま読めない。**
  シートの見出しは全部日本語、`ingest_reviews.py` は `code` / `attendance` / `exam_hard10` と
  **英語で読む**。誰かが手で列名を直していたか、まだ一度も通していないかのどちらか。
  日本語見出し → 内部キーの対応表を足す作業が別に要る（repo 側・未着手）
- **`getActiveSheet()` を直していない。** 書き込み先をタブ名で固定するほうが安全だが、
  今回の変更と混ぜない。**バックアップをタブ複製で取らない理由でもある**（ファイルごとコピーする）
- **`その他（）` を直していない。** 「その他」を選んで自由記述が空のまま送れる
  （`kuchikomi.js:762`）。実データに1件在る。取り込みでは未回答扱いになるので壊れはしないが、
  その1件のテスト情報は使えない。フォーム側で入力必須にする価値あり
- **`HANDOFF.md:2272` の宿題に答えが出た**: シートに `grade` 列は**在る**（「学年」列）。
  中身は「いまの学年」で、「受講年度」列とは別物

### 3. 次の人が最初に打つコマンド

```bash
cat docs/plans/2026-09-03-kuchikomi-difficulty-column-plan.md   # §11 が実行手順
diff tools/gas/kuchikomi_before_20260903.gs tools/gas/kuchikomi_after_20260903.gs
node tools/test_kuchikomi_relay.mjs                             # ブラウザ→Worker の配線
```

### 4. 踏んだ罠

- **「列が無い」の原因を、シートだけ見て決めない。** 送信・中継・GAS・シートの4か所を
  端から突き合わせて、初めて「GAS の配列に1行足りない」に着いた。
  シートに列を足すだけでは**永久に空欄のまま**になっていた
- **配列の順番 = シートの列の順番。** GAS は `setValues` で位置だけで書いている。
  **シートの列挿入とコードの配列は必ずセットで直す。**片方だけだと以降が全部1つずれる
- ★**「新しいデプロイ」を押すと URL が変わる。** `worker/index.js:741` は GAS の URL を
  ベタ書きで持っているので、**口コミが1件も入らなくなる**。
  更新は必ず `デプロイを管理 > ✏️ > 新バージョン`（URL 維持）
- **難易度に `0` を書かない。** テストが無かった回は空欄。`0` を入れると
  「テスト無し」と「難易度0＝簡単」が区別できなくなる（`CLAUDE.md`「欠損値を平均で埋めない」）
- **フォームは難易度を文字列（`"7"`）で送ってくる。** `Number()` を挟まないとシートに
  文字列で入る。`Number(null)` が `0`→偽→`""` になるのを、そのまま「テスト無しは空欄」に使える
