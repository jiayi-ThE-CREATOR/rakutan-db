...
/* 成績評価の内訳・口コミの集計・KOANリンクを組み立てる関数は
   web/assets/detail.js へ出した（2026-09-16）。マイページ（mypage.js）が
   一覧へ飛ばずに同じ詳細をその場で開くため ―― 2か所で組み立てると、
   片方だけ直した日から一覧とマイページで違う詳細が出る。
   このファイルが使うのは下の4つだけ（detailHtml と reviewHtml は詳細と
   口コミモーダル、rvLv と RV_ATT は1件ずつの表示 panelEntry）。 */
const { detailHtml, reviewHtml, rvLv, RV_ATT } = window.rkDetail;
...
/* ── 時間割コード ───────────────────────────
 * KOAN の履修登録で打ち込む6桁。`c.id` がその値そのもので、detail.js の koanUrl() も
...
/* 詳細（4軸バーと信頼度）は開くまで作らない。
   ...
   中身を組み立てるのは detail.js の detailHtml（showDetail から呼ぶ）。 */
...
function toggleSubject(k){
  ...
  /* 授業内容でしぼるのは登録した人だけ（2026-09-22 wang 判断）。左の節は gate.js が
     覆うが、カードと詳細のタグには覆う器が無いので、押されたらここで入口へ送る
     ―― 配点の ✕（#139 の「見えるけど押せない」）と同じやり方。 */
  if (window.rkGate && !window.rkGate.linked()){
    const next = location.pathname + location.search + location.hash;
    location.href = "/line/login?next=" + encodeURIComponent(next);
    return;
