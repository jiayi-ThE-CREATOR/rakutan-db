...
/* 口コミの件数表示。採点に効いているかで見た目を分ける。
   ...
   文言は仕様書の4行（⚠／本文／導線）から1行に変えてある。390px で実測すると
   4行では帯が 77px＝カードの26%を占め、カードで一番強い要素が「口コミがある」
   という注意書きになっていた。1行なら 36px＝14%。松下さんが 2026-08-24 に
   出していた「1行なら17%」と同じ方向。色（--alert-face / --alert-ink）は
   1つも動かしていないので決定Bはそのまま。

   導線（「タップして中身を見る ↓」）を落としたのは、面積が半分になるうえに
   .head 全体が role="button" ですでに押せて導線が二重だったのと、PC では
   詳細が右カラムに出る（決定A）ので「タップ」「↓」の指す先が無いため。
   PC とスマホで文言を出し分ける必要も、幅が変わったときの描き直しも要らなくなる。
