  ...
  const RELEASES = [
    {
      ...
      items: [
        { tag: "new", text: "授業の「配点」でしぼれるようになりました。出席・期末テスト・小テスト・レポートが成績の何%かを指定して探せます" },
        { tag: "new", text: "各科目に KOAN の時間割コードを表示しました。コードの右の「コピー」で写せて、コードでの検索もできます" },
        { tag: "new", text: "小テストの配点を出席と分けて出すようになりました。「出席は緩いけれど毎週小テストがある」授業が見分けられます" },
        { tag: "new", text: "口コミが読みやすくなりました。一覧から直接ひらく大きな画面に変わり、文字も大きくなっています" },
        { tag: "new", text: "一覧から直接「時間割に追加」できるようになりました。詳細を開かなくても押せます" },
        { tag: "new", text: "左の絞り込みを畳めるようになりました。条件と一覧のあいだにある矢印を押すと畳めて、一覧が広がります" },
        { tag: "new", text: "選んだ科目の詳細を ✕（または Esc キー）で閉じられるようになりました。閉じると一覧だけの画面に戻ります（PC）" },
        { tag: "new", text: "About の「ラクハンの強み」を、実際の画面のスクショで紹介するスライドショーにしました" },
        { tag: "improve", text: "「出席なし」などの条件を押すと、対応する配点のつまみが動くようになりました" },
        { tag: "improve", text: "成績評価の表が空だった授業で、シラバス本文に書かれた配点が読めるようになりました。「授業で指示する課題類（80％）と授業での議論への貢献度（20％）」のような記述をそのまま出しています" },
        { tag: "improve", text: "「口コミを読む・書く」「時間割に追加」のボタンを一覧側にまとめ、科目の詳細を短くしました" },
        { tag: "improve", text: "科目を選んでいないあいだは、一覧を画面の中央に出すようになりました。右側が大きく空いて見えていたのを詰めています（PC）" },
        { tag: "fix", text: "「成績評価の内訳」を、シラバスに書いてあるとおりの項目名と数字で出すようになりました。これまでは4つに分類し直して出していたので、発表だけで成績が付く授業が「レポート」と表示されたり、分類できない項目が「不明」に消えたりしていました" },
        { tag: "fix", text: "「期末レポート」「期末課題」をテストとして数えていたのを直しました。レポートだけで成績が付く授業が「レポートのみ」で見つかるようになりました（482→508科目）。これらの授業は、テストではなくレポートとして重さを判定します" },
      ],
    },
    {
      date: "2026-09-02",
